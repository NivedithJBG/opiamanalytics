...
$(function(){
    ...
    $(document).on('click','.editworkgroupbutton',function(){
        ...
        $('#iowGroupId'+idval).hide();
        $('#editworkgroupbutton'+idval).hide();
        $('#editworkgroupname'+idval).show();
        $('#editworkgroupunit'+idval).show();
        $('#editworkgroupquantity'+idval).show();
        $('#editiowgroupid'+idval).show();
        $('#editworktype'+idval).show();
        //$('#editworkgroupname'+idval).focus();
        $('#saveworkgroupbutton'+idval).show();
    });

    $(document).on('click','.editiowgroup',function(){
        var idval=$(this).attr('data-v');
        $('#iowgroupname'+idval).hide();
        $('#editiowgroupname'+idval).show();
        $('#editiowgroup'+idval).hide();
        $('#saveiowgroupbutton'+idval).show();
    });


    $(document).on('click','.saveiowgroupbutton',function(){
        var idval=$(this).attr('data-v');
        var error=0;
        $('.error').hide();
        if($('#editiowgroupname'+idval).val()==''){
            $('#editiowgroupname'+idval).next("span").html('Enter Name').show('slow');
            error=1;
        }
        ...
        if(error==0)
        {
            $.ajax({
                type: 'POST',
                url: '../workgroups1/iowgroupupdate',
                beforeSend : function(){
                    $('#saveiowgroupbutton'+idval).attr("disabled", true);
                },
                dataType: "json",
                data: {groupid:idval,groupname:$('#editiowgroupname'+idval).val()},
                success: function(data){
                    if(data.error=='No')
                    {
                        $('#editiowgroup'+idval).show();
                        $('#iowgroupname'+idval).show();

                        $('#editiowgroupname'+idval).hide();
                        $('#saveiowgroupbutton'+idval).hide();

                        $('#iowgroupname'+idval).text($('#editiowgroupname'+idval).val());    
                    }
                    else
                    {
                        alert(data.errortext);
                    ...
                    $('#saveiowgroupbutton'+idval).attr("disabled", false);
                }
            });
        }
    });
    ...
    $(document).on('click','.saveworkgroupbutton',function(){
        ...
        if($('#editiowgroupid'+idval).val()=='')
        {
            $("#editiowgroupid"+idval).next("span").html('Select Group').show('slow');
            error=1;
        }
        ...
        if(error==0)
        {
            $.ajax({
                ...
                dataType: "json",
                data: {workid:idval,name:$('#editworkgroupname'+idval).val(),worktype:$('#editworktype'+idval).val(),unit: $('#editworkgroupunit'+idval).val(),quantity: $('#editworkgroupquantity'+idval).val(),iowgroupid: $('#editiowgroupid'+idval).val()},
                success: function(data){
                    if(data.error=='No')
                    {
    ...
    $(document).on('click','#saveworkgroup',function(){  
        var error=0;
        $('.error').hide();
        ...
        if($('#iowgroupid').val()=='')
        {
            $("#iowgroupid").next("span").html('Select IOW Group').show('slow');
            error=1;
        }
       /* if($('#workgroupname').val()!='' && WorkGroupNameExists($('#workgroupname').val(),$('#selectedProjectId').val())=='Yes')
        ...
        if(error==0)
        {
            $.ajax({
                ...
                dataType: "json",
                data: {Project_Id:$('#selectedProjectId').val(),workgroupname:$('#workgroupname').val(),iowgroupid:$('#iowgroupid').val(),worktype:$('#worktype').val(),worktypegroup:$('#worktypegroups').val(),unit:$('#workgroupunit').val(),quantity:$('#workgroupquantity').val()},
                success: function(data){
...
$(document).on( "click", ".addIowForm", function(){
...
$(document).on( "click", ".addIowGroupForm", function(){
    $.ajax({
        type: 'POST',
        url: '../workgroups1/iowgrouplist',
        dataType: "json",
        success: function(data){
            $("#iowGroupListContainer").html(data.result);
        }
    });
});

$(document).on('click','#saveIowGroup',function(){

        var error=0;
        $('.error').hide();
        if($('#iowgroupname').val()=='')
        {
            $('#iowgroupname').next("span").html('Enter IOW group name').show('slow');
            error=1;
        }
        ...
        if(error==0)
        {
            $.ajax({
                ...
                beforeSend : function(){
                    $('#saveIowGroup').attr("disabled", true);
                },
                dataType: "json",
                data: {iowgroupname:$('#iowgroupname').val()},
                success: function(data){
                    if(data.error=='No')
                    {
                        $.ajax({
                    ...
                    {
                        $('#iowgroupname').next("span").html(data.errortext).show('slow');
                    }
    
                    $('#saveIowGroup').attr("disabled", false);
                }
            });
        }
    });