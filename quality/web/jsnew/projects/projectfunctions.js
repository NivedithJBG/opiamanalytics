...
$(function(){
    ...
    $('#saveproject').click(function(){
        ...
        if($('#duration').val()=='')
        {
            $("#duration").next("span").html('Select Duration').show('slow');
            error=1;
        }
        ...
        if(error==0){
            $.ajax({
                type:'POST',
                url:'../projects1/create',
                beforeSend:function(){
                    $('#saveproject').attr("disabled", true);
                },
                dataType:'json',
                data: {projectname:$('#projectname').val(),cashaccount:$('#cashaccount').val(),bankaccount:$('#bankaccount').val(),startdate:$('#startdate').val(),duration:$('#duration').val()},
                success:function(data){
                    if(data.error=='No')
                    {
