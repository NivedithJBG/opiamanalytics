...
$(function(){
     ...
     $('#rptattendance').click(function(){ 
        ...
        $('#directLabourTitle').html('Direct Work orders');
        $.ajax({
            type: 'POST',
   ...
   $('#mustroll').click(function(){
        $('#directLabourTitle').html('Attendance Report');
    ...
    $(document).on('click','#atthis',function(){  
        ...
        $('#listmuster').hide();
        $('.edit-form').hide(); 
        $('#directLabourTitle').html('Direct Labour Attendance');
        //var orderid = $(this).attr('data-v');
        $.ajax({
    ...
    $(document).on('change','.ratechange',function(){  
        ...
        //$('.fulldata').each(function(){
           ...
           // var nid = $(this).data('id');
            ...
            var deduction =  parseFloat($('#deduction'+id).val());
            $('#netamount'+id).html(parseFloat(wages - deduction).toFixed(2));
...
$(document).on('click','#musterreport_complete',function(){ 
    $('#musterreport_complete_flag').val(1);
...
function reportAttendance(){
    var error=0;
        ...
        if(error == 1) alert("You have to enter all Employee Name");

        muster_date_split = $('.muster_date').val().split('-');
        muster_date_split = [muster_date_split[1], muster_date_split[0], muster_date_split[2]].join('-');

        var muster_date = formatDate(muster_date_split, 'd-m-y');
        var today = formatDate(new Date(), 'd-m-y');


        if(!muster_date){
            error = 1;
            alert("You have to enter Muster Date");
        }
        else if(muster_date > today){
            error = 1;
            alert("You cannot enter future dates");
        }
                ...
                success: function(data){
                    if(data.error == 'No')
                    {
                     ...
                     //   $('#raiseot'+id).val(otrate);
                        ...
                        $('#raisewages'+id).html(twages.toFixed(2));
                        $('#raisewagesval'+id).val(twages);

                        var deduction =  parseFloat($('#deduction'+id).val());
                        $('#netamount'+id).html(parseFloat(twages - deduction).toFixed(2));
