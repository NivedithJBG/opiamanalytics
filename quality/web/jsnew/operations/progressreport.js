...
$(document).on('click','.resourcereprt',function(){
    var actid = $(this).attr('data-id');
    getResourceReportForm(actid);
});

$(document).on('change','#material_issued_date',function(){
    var actid = $(this).attr('data-id');
    getResourceReportForm(actid, 'Material');
});

$(document).on('change','#attendance_date',function(){
    var actid = $(this).attr('data-id');
    getResourceReportForm(actid, 'Labour');
});

$(document).on('change','#equipment_issued_date',function(){
    var actid = $(this).attr('data-id');
    getResourceReportForm(actid, 'PlantEquip');
});
        ...
        $.ajax({
            ...
            success: function(data){
                if(data.error == 'No')
                {
                    ...
                    $('#task-error-messages').html('<h5>Please enter data for all tasks!</h5>').show().delay(3000).fadeOut('slow');
                }
            }
        });
    }
...
$(document).on('click','.savetaskprogressrpt',function(e){
    ...
    var actid = $(this).attr('data-id');
    var date = $('#select_report_date').val();
    $('#savetaskprogressrpt'+actid).html('Reporting...').attr('disabled', 'disabled');
    //$('#task-error-messages').html('<h5>Reporting...</h5>').show().delay(3000).fadeOut('slow');
    ...
    $('#task_report_status'+actid).val(1);


    $('.resources_at_site_cnt').each(function () {
        if(!$(this).val())
            res_error = 1;
    });

    break_hour_arr = [];
    $('.break_hour').each(function () {
        var id =$(this).attr('data-id');
        var dur = $('#break_hour'+id).val();
        break_hour_arr.push(dur);
    });
    tot_break_hours = getTimeDurationSum(break_hour_arr);

    delay_hour_arr = [];
    $('.cause_of_delay_hours').each(function () {
        delay_hour_arr.push($(this).val());
    });
    tot_delay_hours = getTimeDurationSum(delay_hour_arr);

    //console.log('tot_break_hour -'+ tot_break_hours);
    //console.log('tot_delay_hour -'+ tot_delay_hours);

    if(parseFloat(tot_break_hours) != parseFloat(tot_delay_hours) ){
        delay_error = 1;
    }
    ...
    $('.taskactdur').each(function () {
        ...
        var strtdate = $('#task_date'+id).val(); 
        var enddate  = $('#task_enddate'+id).val();
        var break_hour  = $('#break_hour'+id).val();
        ...
        var d1 = strtdate.split('-');
        var strtdateTime = d1[1] + '/' + d1[0] + '/' + d1[2] + ' ' + strt_time;
        var d2 = enddate.split('-');
        var enddateTime = d2[1] + '/' + d2[0] + '/' + d2[2] + ' ' + end_time;
        var d1 = new Date(strtdateTime);   
        var d2 = new Date(enddateTime);   

        var dateTimeDiff = d2.getTime() - d1.getTime();   
        var minutesdiff = dateTimeDiff / (1000 * 60 );

        if(parseInt(minutesdiff) < 0)
            diff = 1;
        else
            diff = 0;

        if((parseInt(minutesdiff) <= 0 || duration == '') && (parseInt(break_hour) == 0 || break_hour == ''))
            count = 0;
    ...
    if(res_error){
        error = 1;
        $('#task-error-messages').html('<h5>Please enter Resource details!</h5>').show().delay(3000).fadeOut('slow');
    }
    
    if(delay_error){
        error = 1;
        $('#task-error-messages').html('<h5>Please enter Cause of Delay details!</h5>').show().delay(3000).fadeOut('slow');
    }
    
    if(count==0){
        $('#task-error-messages').html('<h5>Please enter start and end time for all tasks!</h5>').show().delay(3000).fadeOut('slow');
        $('#savetaskprogressrpt'+actid).removeAttr('disabled');
    }
    else{
        if(diff == 1){
            $('#task-error-messages').html('<h5>Start time is greater than end time!</h5>').show().delay(3000).fadeOut('slow');
            ...
            $.ajax({
                ...
                success: function(data){
                    if(data.error == 'No')
                    {
                        $('#task-success-messages').html('<h5>Task Reported Successfully!</h5>').show().delay(3000).fadeOut('slow', function(){
                            $('#progress_act_reprt').trigger('click');
                            $.ajax({
                                type: 'POST',
                                url: '../report/scheduleprogresstasklist',
    ...
    $('#savetaskprogressrpt'+actid).html('Report').removeAttr('disabled');
...
function changeCauseofDelayMode(){
    console.log('Testing...');
    mode = 'disabled';
    $('.break_hour').each(function () {
        if($(this).val()){
            break_hour_diff = time_diff($(this).val(), '00:00');
            break_hour_arr = break_hour_diff.split(":");
            if(break_hour_arr[0] > 0 || break_hour_arr[1] > 0)
                mode = ''
        }
    });
    $('.taskwastedur').each(function () {
        console.log('taskwastedur' + $(this).val());
        if($(this).val()){
            waste_hour_diff = time_diff($(this).val(), '00:00');
            waste_hour_arr = waste_hour_diff.split(":");
            if(waste_hour_arr[0] > 0 || waste_hour_arr[1] > 0)
                mode = ''
        }
    });
    $('.task_stoppage_time').each(function () {
        console.log('task_stoppage_time' + $(this).val());
        if($(this).val()){
            stoppage_time_diff = time_diff($(this).val(), '00:00');
            stoppage_time_arr = stoppage_time_diff.split(":");
            if(stoppage_time_arr[0] > 0 || stoppage_time_arr[1] > 0)
                mode = ''
        }
    });
    ...
    if(mode)
        $('#cause_of_delay').attr('disabled', 'disabled');
    else   
        $('#cause_of_delay').removeAttr('disabled');
}
...
$(document).on( "click", ".cause_of_delay_btn", function(){
    $.ajax({
        type: 'POST',
        url: '../report/causeofdelaylist',
        dataType: "json",
        success: function(data){
            $("#causeOfDelayListContainer").html(data.result);
        }
    });
});

$(document).on('click','#saveCauseofDelay',function(){

        var error=0;
        $('.error').hide();
        if($('#causeofdelay').val()=='')
        {
            $('#causeofdelay').next("span").html('Enter Cause of Delay').show('slow');
            error=1;
        }
        ...
        if(error==0)
        {
            $.ajax({
                type: 'POST',
                url: '../report/causeofdelaycreate',
                beforeSend : function(){
                    $('#saveCauseofDelay').attr("disabled", true);
                },
                dataType: "json",
                data: {causeofdelay:$('#causeofdelay').val()},
                success: function(data){
                    if(data.error=='No')
                    {
                        $.ajax({
                    ...
                    {
                        $('#causeofdelay').next("span").html(data.errortext).show('slow');
                    }
                    $('#saveCauseofDelay').attr("disabled", false);
                }
            });
        }
    });
...
$(document).on('click','.editcauseofdelay',function(){
        var idval=$(this).attr('data-v');
        $('#causeofdelay'+idval).hide();
        $('#editcauseofdelay'+idval).show();
        $('#savecauseofdelaybutton'+idval).show();
    });


    $(document).on('click','.savecauseofdelaybutton',function(){
        var idval=$(this).attr('data-v');
        var error=0;
        $('.error').hide();
        if($('#editcauseofdelay'+idval).val()==''){
            $('#editcauseofdelay'+idval).next("span").html('Enter Name').show('slow');
            error=1;
        }
        ...
        if(error==0)
        {
            $.ajax({
                type: 'POST',
                url: '../report/causeofdelayupdate',
                beforeSend : function(){
                    $('#savecauseofdelaybutton'+idval).attr("disabled", true);
                },
                dataType: "json",
                data: {causeofdelayid:idval,causeofdelay:$('#editcauseofdelay'+idval).val()},
                success: function(data){
                    if(data.error=='No')
                    {
                        $('#editcauseofdelay'+idval).show();
                        $('#causeofdelay'+idval).show();

                        $('#editcauseofdelay'+idval).hide();
                        $('#savecauseofdelaybutton'+idval).hide();

                        $('#causeofdelay'+idval).text($('#editcauseofdelay'+idval).val());    
                    }
                    else
                    {
                    ...
                    $('#savecauseofdelaybutton'+idval).attr("disabled", false);
                }
            });
        }
    });
