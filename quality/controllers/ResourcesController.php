...
/*			array('allow', // allow admin user to perform 'admin' and 'delete' actions
  ...
  public function actionCreate()
  {
    ...
    //echo 'hai'; exit;
      ...
      if (isset($_POST['resourcename'])) {
          ...
          if($_POST['restype'] == 26){
            if(isset($_POST['asset_register_item_id']))
                $model->asset_register_item_id = $_POST['asset_register_item_id'];

            if(isset($_POST['purchase_date']))
                $model->purchase_date = date('Y-m-d', strtotime($_POST['purchase_date']));

            if(isset($_POST['reg_number']))
                $model->reg_number = $_POST['reg_number'];

            if(isset($_POST['id_number']))
                $model->id_number = $_POST['id_number'];

            if(isset($_POST['capacity']))
                $model->capacity = $_POST['capacity'];
          }

          if($_POST['resourceGroup']!='none'):
              $model->Resource_group_Id = $_POST['resourceGroup'];
          endif;
          ...
          if($_POST['lead_time']!=''):
              $model->lead_time = $_POST['lead_time'];
          endif;

          if($_POST['credit_period']!='none'):
              $model->credit_period = $_POST['credit_period'];
          endif;
          ...
          $model->current_project = 12;//Office Corporate
  ...
  public function actionSearch() 
  {
      ...
      $restype_id = '';
      $restypename='';
      $sql = "SELECT a.Resource_Id,a.Name,Unit,a.Price,a.ResourceType_Id,a.Vendor_Id,a.Project_Id,a.sortorder,a.Updated_On,b.Name AS restype,
              c.Name AS vendor,c.Email,c.Phone,c.Address,c.City,a.Resource_Acc_Id,a.Resource_Location,d.name,a.Resource_group_Id,a.Quantity, 
              a.asset_register_item_id  
              FROM resources AS a
              LEFT JOIN resourcetype AS b ON a.ResourceType_Id=b.ResourceType_Id
              LEFT JOIN vendors AS c ON c.Vendor_Id=a.Vendor_Id
              LEFT JOIN accounts_item AS d ON d.id=a.Resource_Acc_Id
      ...
      if(isset($_POST['resourcetype']) && $_POST['resourcetype'] != 'none'):
          $sql .= "AND a.ResourceType_Id='" . $_POST['resourcetype'] . "'";
      ...
      if($restype_id == 26)//P&M
        $sql .= " AND a.asset_register_item_id !=0 GROUP BY a.asset_register_item_id";
      else
        $sql .= " GROUP BY a.Name";
      $sql .= " ORDER BY a.Resource_Id  DESC";
      ...
      $resourceGroupList=ResourceGroup::find()->where(['ResourceType_Id'=>$_POST['resourcetype']])->andWhere(['status' => 0])->all();

      $resGroupDropDown = '<option value="none">Select Resource Group</option>';
      if($resourceGroupList){
        foreach ($resourceGroupList as $resourceGroup) {
          $resGroupDropDown .= '<option value="'.$resourceGroup->Resource_group_Id.'">'.$resourceGroup->Resource_group_Name.'</option>';
        }
      }
      ...
      if (count($dataProvider) > 0):
          ...
          foreach ($dataProvider AS $key => $data):
              ...
              if($casesenstve!=0){
                ...
                if($restype_id == 26){//P&M
                  $asset_register_item = AssetRegisterItem::findOne($data['asset_register_item_id']);
                  $resource_Name = $asset_register_item['item_name'];
                }
                else
                        ...
                        <table class="table reshds">
                          <tr style="background: #f5f5f5;font-size:15px;" id="resourcerow' . $data['Resource_Id'] . '" class="vendor-column reshead resource' . $data['Resource_Id'] . '" data-id="' . $data['Resource_Id'] . '">
                            ...
                            if($restype_id != 26){//P&M
                             ...
                             $resource.=' <div class="icon-groups">
                                <button type="button" class="addvendor" value="' . $data['Resource_Id'] . '" id="addvendor' . $data['Resource_Id'] . '" title="Add Vendor"><a  href="#" class="icon-add add-vendor-at-resource"></a></button>                          
                              </div>';
                            }
              ...
              if($restype_id == 26)//P&M
                $sql = 'SELECT * FROM resources WHERE 1 AND asset_register_item_id = '.$data['asset_register_item_id'].' AND ResourceType_Id= 26 AND Status=0 AND pricing_status=0 ';
              else
                $sql = 'SELECT * FROM resources WHERE Name LIKE "'.$resnamefinal.'" AND Status=0 AND pricing_status=0 AND ResourceType_Id= '.$restype_id;
                      ...
                      if(isset($_POST['brand']) && $_POST['brand']!='none'){
                          ...
                          if($vendorbrand1):
                            ...
                            $resource.='
                              ...
                              if($restype_id == 26){//P&M
                               $resource.=' <td colspan="3" style="width:10%;">
                                  <div class="icon-groups">                 
                                      <button type="button" class="editresourcebuttons" style="display: inline-block;"  value="' . $data['Resource_Id'] . '" id="editresourcebuttons' . $data['Resource_Id'] . '" data-vendor="'.$data['Vendor_Id'].'" data-mode="old" title="Edit Vendor"> <a href="#" class="icon-pencil editVendor-pencil" title="Edit Resource"></a></button>
                                      <button type="button" class="deletresourcebutton" value="' . $data['Resource_Id'] . '" id="delet_resourcebutton' . $data['Resource_Id'] . '" data-mode="master" title="Delete Resource">
                                        <a title="Delete Resource" href="#" class="icon-trash1"></a>
          ...
          $arr['resGroupDropDown'] = $resGroupDropDown;
      else:
 ...
 /*  public function actionDeleteresource()
  ...
  public function actionEditresource()
  {
      ...
      $resGroups = ResourceGroup::find()->where(['ResourceType_Id' => $data['ResourceType_Id'], 'status' => 0])->all();
      ...
      if($data):
              ...
              <div class="col-md-10">
                ...
                $datarows.= '
                  ...
                  <div class="col-md-3" style="width: 160px;">
                    <div class="form-title" style="display:block; margin-top:15px; padding-left: 20px; margin-bottom:0px;  text-align:left;">Resource Group: 

                    </div> 
                  </div>

                  <div class="col-md-4" style="display:block; margin-top:11px; font-size:17px; margin-bottom:0px; margin-top:9px; text-align:left;padding-right: 0px;width: 250px;">
                      <select class="form-control" id="resourceGroup" name="resourceGroup"  >
                              <option value="none">Select Resource Type</option>';

                          foreach ($resGroups AS $resGroup):
                              if ($data['Resource_group_Id'] == $resGroup->Resource_group_Id):
                                  $seletedtype = 'Selected="selected"';
                              else:
                                  $seletedtype = '';
                              endif;
                              $datarows.= "
                                    <option value='" . $resGroup->Resource_group_Id . "' " . $seletedtype . ">" . $resGroup->Resource_group_Name . "</option>";
                          endforeach;
        ...
        if($data['ResourceType_Id'] == 26){ //P&M
          $datarows.= '<div class="row">
                    <div class="col-md-1"></div>
                      <div class="row asset_reg_row col-md-10" style="">
                          <div class="col-md-4">
                            <div class="form-group">
                              <label>Asset Register Item<span style="color:red;"> *</span></label>
                                        <select class="form-control" id="edit_asset_register_item_id" name="asset_register_item_id" >
                                          <option value="">Select Asset Register Item</option>';
                                          
                                      $itemList = AssetRegisterItem::find()->where(['Status'=>1])->all();
                                        foreach($itemList AS $list):  
                                          if ($data['asset_register_item_id'] == $list->id)
                                              $seletedtype = 'Selected="selected"';
                                          else
                                              $seletedtype = '';
                                          $datarows.=  "<option data-eq-type='".$list->type."' ".$seletedtype." value='".$list->id."'>".$list->item_name."</option>";
                                        endforeach;
                        ...
                        $datarows.= '</select>
                          ...
                          <div class="col-md-2">
                            ...
                            <input class="form-control" type="text" id="edit_purchase_date" name="purchase_date" placeholder="Date of Purchase" value="'.$data['purchase_date'].'">
                          </div>
                          <div class="col-md-2">
                            <label>Registration Number<span style="color:red;"> *</span></label>
                            <input class="form-control" type="text" id="edit_reg_number" name="reg_number" placeholder="Registration Number"  value="'.$data['reg_number'].'">
                            <span class="error" style="display: none;"></span>
                          </div>
                          <div class="col-md-2">
                            <label>Identification Number<span style="color:red;"> *</span></label>
                            <input class="form-control" type="text" id="edit_id_number" name="id_number" placeholder="Identification Number"  value="'.$data['id_number'].'">
                            <span class="error" style="display: none;"></span>
                          </div>
                          <div class="col-md-2">
                            <label>Capacity<span style="color:red;"> *</span></label>
                            <input class="form-control" type="text" id="edit_capacity" name="capacity" placeholder="Capacity"  value="'.$data['capacity'].'">
                            <span class="error" style="display: none;"></span>
                          </div>
              ...
              <div class="col-md-10">
                <div class="col-md-4">                    
                  <div class="form-group">
                      ...
                      <input class="form-control" type="text" id="editresource_lead_time" name="lead_time" placeholder="Lead Time"  value="'.$data['lead_time'].'">
                      <span class="error" style="display: none;"></span>
                  </div>            
                </div>
                ...
                <div class="col-md-4">                    
                  <div class="form-group">
                      ...
                      <select id="credit_period" name="credit_period" class="form-control">
                        <option value="none">Select</option>
                        <option value="0" '.(($data['credit_period'] == '0') ? 'selected' : '').' >Instantly</option>
                        <option value="15" '.(($data['credit_period'] == '15') ? 'selected' : '').'>15 Days</option>
                        <option value="30" '.(($data['credit_period'] == '30') ? 'selected' : '').'>30 Days</option>
                        <option value="45" '.(($data['credit_period'] == '45') ? 'selected' : '').'>45 Days</option>
                        <option value="60" '.(($data['credit_period'] == '60') ? 'selected' : '').'>60 Days</option>
                        <option value="90" '.(($data['credit_period'] == '90') ? 'selected' : '').'>90 Days</option>
                      </select>
                      <span class="error" style="display: none;"></span>
                  </div>            
                </div>
  ...
  public function actionResourceupdate()
  {
    ...
    foreach($allresources as $allresource):
      ...
      $resource = Resources::findOne($allresource['Resource_Id']);
        ...
        if($_POST['resourcetype'] == 26){
          if(isset($_POST['asset_register_item_id']))
              $resource->asset_register_item_id = $_POST['asset_register_item_id'];

          if(isset($_POST['purchase_date']))
              $resource->purchase_date = date('Y-m-d', strtotime($_POST['purchase_date']));

          if(isset($_POST['reg_number']))
              $resource->reg_number = $_POST['reg_number'];

          if(isset($_POST['id_number']))
              $resource->id_number = $_POST['id_number'];
      ...
      if($_POST['lead_time']!=''):
          $resource->lead_time = $_POST['lead_time'];
      endif;

      if($_POST['credit_period']!='none'):
          $resource->credit_period = $_POST['credit_period'];
      endif;



      if($_POST['resourceGroup']!='none'){
          $resource->Resource_group_Id = $_POST['resourceGroup'];
  ...
  public function actionResestdata()
  {
    ...
    $sql            = "SELECT v.Vendor_Id, v.Name
                        FROM vendors AS v
                        INNER JOIN resources AS r ON  r.Vendor_Id = v.Vendor_Id
                        WHERE r.Vendor_Id != 0 AND r.Name like 'Diesel'";
    $dieselVendors    = $connection->createCommand($sql)->query()->readAll();
    $dieselVendorList = '';
    if($dieselVendors){
      foreach ($dieselVendors as $dieselVendor)
        $dieselVendorList .="<option class='dieselVendor' value='".$dieselVendor['Vendor_Id']."'>".$dieselVendor['Name']."</option>";
    }

    $sql              = "SELECT v.Vendor_Id, v.Name
                        FROM vendors AS v
                        INNER JOIN resources AS r ON  r.Vendor_Id = v.Vendor_Id
                        WHERE r.Vendor_Id != 0 AND r.Name like 'Petrol'";
    $petrolVendors    = $connection->createCommand($sql)->query()->readAll();
    $petrolVendorList = '';
    if($petrolVendors){
      foreach ($petrolVendors as $petrolVendor)
        $petrolVendorList .="<option class='petrolVendor' value='".$petrolVendor['Vendor_Id']."'>".$petrolVendor['Name']."</option>";
    }
        ...
        $datarows='
                <div class="col-md-6">
                  <div class="row">
                    ...
                    <div class="col-md-3 type">
                      <label>Vendor</label>
                      <select class="form-control" name="fuel_vendor" data-id='.$childdata['Resource_Id'].' id="fuel_vendor'.$childdata['Resource_Id'].'">
    ...
    else
    {
      ...
      $tasklist = '';
      if($SchleActvty = Scheduleactivities::find()->where(["activity_id" =>  $_POST['activity_id']])->one()){
        if($schTasks = TasksNew::find()->where(["activity_Id" =>  $SchleActvty->id])->all()){
          foreach ($schTasks as $task) {
            $tasklist .= '<option value="'.$task->task_Id.'" >'.$task->Name.'</option>';
          }
        }
      }
                  ...
                  <div class="row">
                    ...
                    <div class="col-md-6 type">
                      <div class="form-group">
                        ...
                        <label>
                            <input class="form-control"  mbsc-input id="task_id-select-input-'.$childdata['Resource_Id'].'" placeholder="Please select..." data-dropdown="true" data-input-style="outline" data-label-style="stacked" data-tags="true" />
                        </label>


                        <select class="form-control " multiple  style="height: 40px;" id="task_id'.$childdata['Resource_Id'].'" >
  ...
  public function actionResplantestdata()
  {
    ...
    else
    {
      ...
      $totQty = Resources::find()->Where(['asset_register_item_id'=>$childdata['asset_register_item_id']])->count();

      $qty_options = '';
      if($totQty){
        $qty_options.='<select class="form-control" id="estimate-dep-qty'.$childdata['Resource_Id'].'">';
        for ($i=1; $i <=$totQty ; $i++)
          $qty_options.='<option value="'.$i.'">'.$i.'</option>';
        $qty_options.='<select>';
      }
      else{
          $qty_options = '<span class="error">No Resources Added</span>';
      }
                ...
                <div class="col-md-12">
                  <div class="row">
                   <!-- <div class="col-md-3 type">
                    ...
                    <div class="col-md-2 type">
                      <label>Quantity</label>
                      <!--<input type="number" id="estimate-dep-qty'.$childdata['Resource_Id'].'" class="form-control" > -->
                      '.$qty_options.'
                    </div>
  ...
  public function actionMasterestdata()
  {
    ...
    public function actionResourcegrouplist(){

        $resGroups = ResourceGroup::find()->where(['ResourceType_Id' => $_POST['resType'], 'status' => 0])->all();
        ...
        if($resGroups){
            ...
            foreach ($resGroups as $key => $resGroup) {
                ...
                $datarows .= '<div style="bottom: 15px;" class="col-md-12 datalists scheduleitemcontent" id="" >
                                ...
                                <div class="row  datslis" style="cursor: pointer;" id="">
                                    ...
                                    <div class="col-md-7 type">
                                        <span id="resgroupname'.$resGroup->Resource_group_Id.'">'.$resGroup->Resource_group_Name.'</span>
                                        <input class="form-control editresgroupname" type="text" id="editresgroupname'.$resGroup->Resource_group_Id.'" value="'.$resGroup->Resource_group_Name.'" style="display:none;">
                                        <span class="error"></span>
                                    </div>
                                    <div class="col-md-4 icon-groups">
                                        <a class="btn btn-primary icon-pencil editresgroup" id="editresgroup'.$resGroup->Resource_group_Id.'" data-v="'.$resGroup->Resource_group_Id.'" title="Rename Resource Group" href="#"></a>
                                        <a class="btn btn-primary icon-save saveresgroupbutton"  style="display:none;" data-v="'.$resGroup->Resource_group_Id.'" id="saveresgroupbutton'.$resGroup->Resource_group_Id.'" title="Save" href="#"></a>
                                    </div>
                                </div>
                            </div>';
            }
            ...
            $datarows='<div class="row"><div class="col-md-12"><div class="text-center">No Resource group found!</div></div></div>';
    ...
    public function actionResgroupcreate()
    {
        if(!$_POST['resType']){
            $arr = array('error'=>'Yes','errortext'=>'Resource type not selected!');
            return json_encode($arr);
        }

        $resourceGroup = ResourceGroup::find()->where(['ResourceType_Id' => $_POST['resType'],'Resource_group_Name' => $_POST['resgroupname']])->one();
        if($resourceGroup){
            $arr = array('error'=>'Yes','errortext'=>'Resource group already added for this project!');
            return json_encode($arr);
        }

        if(isset($_POST['resgroupname']) && $_POST['resgroupname'])
        {
            $model=new ResourceGroup;
            $model->Resource_group_Name=$_POST['resgroupname'];
            $model->ResourceType_Id=$_POST['resType'];
            if($model->save(false)):
                $arr = array('error'=>'No');
                return json_encode($arr);
    ...
    public function actionResgroupupdate()
    {

        $resGroup = ResourceGroup::findOne($_POST['groupid']);
        if(!$resGroup){
            $arr = array('error'=>'Yes','errortext'=>'Resource group not exist!');
            return json_encode($arr);
        }

        if(isset($_POST['groupname']) && $_POST['groupname'])
        {
            $resGroup->Resource_group_Name=$_POST['groupname'];
            if($resGroup->save(false)):
                $arr = array('error'=>'No');
                return json_encode($arr);
            else:
                $arr = array('error'=>'Yes','errortext'=>'Cannot add now. Please try again later');
