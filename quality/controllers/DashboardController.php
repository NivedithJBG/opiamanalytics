...
class DashboardController extends Controller
{
    ...
    public function actionActivitycost()
    {
        ...
        if($projuser){
            ...
            /*$sqltemvals1 = "SELECT * FROM workgroups_new WHERE Project_Id=' ".$projectid." ' AND Status=0 AND pricing_status =0 ORDER BY sortorder ASC";
            $command = $connection->createCommand($sqltemvals1);
            $dataReader1 = $command->query();
            $structures = $dataReader1->readAll();
            $projectcost=0;
            $projectestcost=0;
             if(count($structures)>0){
                foreach($structures AS $structure):
                    $sql = "SELECT *  FROM `workgroup_activities_new` WHERE `wbs_id` = ".$structure['Workgroup_Id']." AND `estimate` = 1 AND `project_Id` = ".$projectid." AND `pricing_status` = 0";
                    //echo $sql;exit;
                    $command = $connection->createCommand($sql);
                    $dataReader1 = $command->query();
                    $iowactivities = $dataReader1->readAll();
                    $activitycost=0;
                    $estactivitycost=0;
                    if (count($iowactivities)>0) {
                        foreach ($iowactivities as $key => $iowactivity) {
                            $dataitems=$this->getactivityactcost($projectid,$iowactivity['id'],$iowactivity['wbs_id']);
                            $estactivitycost+=$dataitems['estactivitycost'];
                        }
                    }
                    $projectestcost+=$estactivitycost;
                endforeach;
            }

            $pricesum = $projectestcost;*/
    ...
    public function actionProjectcompletedate()
    {
        ...
        /*$datediff1 = strtotime($activityenddate) - strtotime($activitystartdate);
        $diffduration = round($datediff1 / (60 * 60 * 24)) + 1;

        $datediff2 = strtotime($activityenddate) - strtotime($activitystartdate);
        $totduration = round($datediff2 / (60 * 60 * 24)) + 1;

        $barduration = ($diffduration/$totduration)*100;

        $barpercentage = round($barduration);*/
    ...
    /* Activity Actual Cost Calculation Start */

    /*public function actionActivitybasedcost()
    {
        $connection = Yii::$app->db;
        $uid = Yii::$app->user->Id; 
        $projuser = ProjuserSelection::find()->where(['userid' => $uid])->one();
        ...
        $sqltemvals1 = "SELECT * FROM workgroups_new WHERE Project_Id=' ".$projectid." ' AND Status=0 AND pricing_status =0 ORDER BY sortorder ASC";
        //$sqltemvals1 = "SELECT * FROM workgroups_new WHERE Workgroup_Id=69 AND Project_Id=56 AND Status=0 AND pricing_status =0 ORDER BY sortorder ASC";
        $command = $connection->createCommand($sqltemvals1);
        $dataReader1 = $command->query();
        $workgroups = $dataReader1->readAll();
        ...
        if($workgroups){
            ...
            foreach($workgroups AS $workgroup):
                ...
                $sql = "SELECT * FROM workgroup_activities_new WHERE project_Id='".$workgroup['Project_Id']."' AND wbs_id='".$workgroup['Workgroup_Id']."' AND estimate=1 AND pricing_status=0 ORDER BY sortorder ASC ";
                $command = $connection->createCommand($sql);
                $dataReader = $command->query();
                $wbsactivities = $dataReader->readAll();
                ...
                $total =0;
                if($wbsactivities){
                    if (!in_array($workgroup['Workgroup_Id'], $iowids))
                    {
                        array_push($iowids, $workgroup['Workgroup_Id']);

                        array_push($iownames, $workgroup['Name']);

                        $iowcount++;

                    }
                    $actdatarows1 = 'data.addRows([';
                    //$datarows .= '<tr> <td colspan="7"><b>' . $workgroup['Name'] . '</b></td></tr>';
                    $count = 0;
                    foreach($wbsactivities AS $wbsactivity):
                        $estimateresources=PricingEstimateResourcesNew::find()->where(['activity_id' => $wbsactivity['id']])->andWhere(['project_id' =>$projectid])->andWhere(['pricing_status' =>0])->all();
                        $resamttotal=0;
                        $resnamearr=array();
                        if($estimateresources):
                            foreach($estimateresources as $estimateresource):
                                ...
                                endforeach;
                                //if($resource->ResourceType_Id==16 or $resource->ResourceType_Id==20 or $resource->ResourceType_Id==21 or $resource->ResourceType_Id==23):
                                ...
                                if (!in_array($resource->Name, $resnamearr))
                                {
                                    ...
                                    if(isset($resdata['issuqty']) && isset($resdata['avg_rate'])){
                                        $amount=$resdata['issuqty'] * $resdata['avg_rate'];
                                    }
                                    else{
                                        $amount= 0;
                        ...
                        $total = $total + $amount;
                        $count++;
                        if($estimateresources && $amount!=0){
                            $actdatarows1.= "['".$wbsactivity['activity_Name']."', ".$amount."],";
                        }
                    endforeach; 
                    $datarows.= "['".$workgroup['Name']."', ".$total."],";
                    $iowlist.= '<div class="col-md-6 iowspace">'.$workgroup['Name'].' - '.number_format($total,2).'</div>';
                    //IOW Total -> number_format($total,2)
                    $actdatarows1.= "]);";

                    array_push($actdatarows, $actdatarows1);
                }
                $wbtotal = $wbtotal + $total;
            endforeach;
        }

        $datarows.= "]);";

        //echo $actdatarows; exit;

        //print_r($iowids); exit;
        
        $arr = array('result' => $datarows,'iowlist'=> $iowlist,'actdatarows' => $actdatarows,'iowids' => $iowids,'iownames' => $iownames, 'iowcount'=> $iowcount, 'actualcost'=> $wbtotal, 'error' => 'No');
    ...
    /*public function getactivityactcost($projectid,$estactid,$wbsid)
    {
        if($projectid == 56){
        ...
        $reportedacts=ActivityReportWrkactvty::find()->where(['iow_id' => $wbsid])->andWhere(['activity_id' => $estactid])->andWhere(['projectid' => $projectid])->andWhere(['status' => 0])->all();
        $totrepqty=0;
        foreach($reportedacts AS $reportedact):
            $totrepqty+=$reportedact['quantity'];
        endforeach;
        //echo "totrepqty: ".$totrepqty;exit;
        $estimateresources=PricingEstimateResourcesNew::find()->where(['activity_id' => $estactid])->andWhere(['project_id' => $projectid])->andWhere(['pricing_status' => 0])->all();
        //echo "estimateresources: ".count($estimateresources);exit;
        $resourcecost=0;
        $estactivitycost=0;
        $rescount=0;
        if (count($estimateresources)>0) {
            foreach ($estimateresources as $key => $estimateresource) {
                $resunitamount = $estimateresource['quantity'] * $estimateresource['rate'];
                //echo "estresunitamount: ".$resunitamount;exit;
                $activitycost=$resunitamount * $totrepqty;
                //echo "estrescost: ".$activitycost;exit;
                $estactivitycost+=$activitycost;

                $rescount++;
            }
            //echo "rescount: ".$rescount;exit;
            //echo "resourcecost: ".$resourcecost;exit;
            $average_rate=$resourcecost / $rescount;
            
            $actactivitycost=$resourcecost;
        }
        //echo "actualamount: ".$actualamount;exit;
        $result = array('estactivitycost'=>$estactivitycost);
        return $result;

    }*/

    /* Activity Actual Cost Calculation End */
    ...
    /*public function actionProjectresourcecost()
    {
        ...
        //echo $stocktotal;exit;
        $sql="SELECT Project_Id,Name FROM projects WHERE Project_Id='".$projectid."' ";
        $command=$connection->createCommand($sql);
        $dataReader=$command->query();
        $project=$dataReader->read();
        //$peinfo="<p style='text-align: left'>Project Expenditure of ".$project['Name']."</p>";
        ...
        foreach($subgroups AS $subgroup):
            ...
            if($accounts):
                ...
                if($subgroup['id']==57):
                    $subgrpamount=$grptotal - $stocktotal;
                else:
                    $subgrpamount=$grptotal;
                    //$amountdetails=number_format((float)abs($subgrpamount), 2);
                endif;

                if($dataval['count']>0):

                    $amt=round(abs($subgrpamount), 2);

                    $datarows.= "['".$subgroup['name']."', ".$amt."],";

                endif;

                $projexptot=$projexptot + abs($subgrpamount);
            endif;
        endforeach;
    ...
    public function actionAccountheadcost()
    {
        $connection = Yii::$app->db;
        $startdate='';
        $enddate='';
        ...
        $restypids = array();
        $restypcount = 0;
        //$subgrpid=$_POST['subgrpid'];
        //$stocktotal = $this->Getstockamount($projectid);
        //echo "estimatesub: ".$estimatesub;exit;
        $sql="SELECT id,name FROM accounts_sub WHERE master_id='1' ORDER BY sortorder ASC,id DESC";
        $command=$connection->createCommand($sql);
        $dataReader=$command->query();
        $subgroups=$dataReader->readAll();
        if($subgroups):
            foreach($subgroups as $subgroup):
                ...
                $estimatesub = $this->Getestimateamount($project->Project_Id,$subgrpid);
                $subgroupinfo="<p style='text-align: left'>".$subgroup['name']."</p>";
                $grptotal=0;
                $count=0;
                $sql1="SELECT a.subgrp_id,b.id,b.name FROM subgroup_accounts AS a INNER JOIN accounts_item AS b ON a.account_id=b.id WHERE a.subgrp_id='".$subgrpid."' AND accountschedule_id=0 AND b.account_type IN (5,8,6)";
                $command=$connection->createCommand($sql1);
                $dataReader=$command->query();
                $accounts=$dataReader->readAll();
                if($accounts):
                    foreach($accounts AS $key=>$accounthead):
                        ...
                        if($netamount!=0):
                            ...
                            $datarows.= "['".$accounthead['name']."', ".$netamount."],";

                            if (!in_array($subgroup['id'], $restypids))
                            {
                                array_push($restypids, $subgroup['id']);

                                array_push($restypnames, $subgroup['name']);
        ...
        $arr = array('result'=>$datarows, 'restypids'=>$restypids,'restypnames'=>$restypnames, 'error'=>'No');
        // echo $arr;
        return json_encode($arr);
    }

    public function Getestimateamount($projectid,$subgrpid)
    {
        $connection = Yii::$app->db;
        $sql="SELECT id,name,ResourceType_Id FROM accounts_sub WHERE master_id='1' AND id=".$subgrpid." ORDER BY sortorder ASC,id DESC";
        $command=$connection->createCommand($sql);
        $dataReader=$command->query();
        $subgroup=$dataReader->read();
        $amount = 0;
        //echo $subgroup['ResourceType_Id'];exit;
        $estimateresources=PricingEstimateResourcesNew::find()->where(['resourcetype_Id' => $subgroup['ResourceType_Id']])->andWhere(['project_id' => $projectid])->andWhere(['pricing_status' => 0])->all();
        foreach($estimateresources AS $estimateresource):
            $workgroupact=WorkgroupActivitiesNew::find()->where(['id' => $estimateresource->activity_id])->andWhere(['project_Id' => $estimateresource->project_id])->andWhere(['estimate' => 1])->andWhere(['pricing_status' => 0])->one();
            $workgroup = WorkgroupsNew::findOne($workgroupact->wbs_id);
            if($workgroup->Status==0){
                $reportedacts=ActivityReportWrkactvty::find()->where(['iow_id' => $workgroup->Workgroup_Id])->andWhere(['activity_id' => $estimateresource->activity_id])->andWhere(['projectid' => $estimateresource->project_id])->andWhere(['status' => 0])->all();
                $totrepqty=0;
                foreach($reportedacts AS $reportedact):
                    $totrepqty+=$reportedact['quantity'];
                endforeach;
                //echo $totrepqty;exit;
                $unitresrate= $estimateresource->quantity * $estimateresource->rate;
                $amount+=$unitresrate * $totrepqty;
                //echo ($estimateresource->actual_amount * $totrepqty)."<br>";
            }
        endforeach;
        return $amount;
    }
    ...
    public function actionProcurementchart()
    {
        $connection = Yii::$app->db;
        $uid = Yii::$app->user->Id; 
        $projuser = ProjuserSelection::find()->where(['userid' => $uid])->one();
        ...
        $datarows_materials = 'data.addRows([';
        ...
        if($resourcetypes):
            ...
            foreach($resourcetypes AS $resourcetype):
                ...
                if($resourcetype->ResourceType_Id==16):
                    ...
                    if($structures){
                        ...
                        /*if($restypecost!=0){
                            $datarows_materials.= "['".$resourcetype->Name."', ".$restypecost."],";
                        }*/
                    }  


                    /* procured amount calculation start */
                    ...
                    $sql1="select a.project_id,a.invoice_id,b.resource_id,c.Resource_group_Id,c.Name from invoice AS a INNER JOIN invoice_resources AS b ON a.invoice_id=b.invoice_id INNER JOIN resources AS c ON b.resource_id=c.resource_id WHERE a.status!=3 AND c.ResourceType_Id=16 ".$condition." ";
                    //echo $sql1;exit;
                    $command = $connection->createCommand($sql1);
                    $dataReader = $command->query();
                    $childresitems = $dataReader->readAll();
                    ...
                    if(count($childresitems)>0):
                        foreach($childresitems AS $key1=>$data1):

                            $sql1="select a.project_id,a.invoice_id,b.resource_id,c.Resource_group_Id from invoice AS a INNER JOIN invoice_resources AS b ON a.invoice_id=b.invoice_id INNER JOIN resources AS c ON b.resource_id=c.resource_id WHERE a.status!=3 AND c.ResourceType_Id=16 AND c.Name LIKE '".$data1['Name']."' ".$condition." group by b.resource_id ORDER BY b.resource_id ASC";
                            //echo $sql1;exit;
                            $command = $connection->createCommand($sql1);
                            $dataReader = $command->query();
                            $stockresitems = $dataReader->readAll();
                        ...
                        $purchasetotal+= $purchaseamounttot;

                    endif;

                    /* procured amount calculation end */

                    $estimatetotal = round($pricesum);

                    $proctotal = round($purchasetotal);

                    $numlength = strlen((string)$estimatetotal);
                    //echo "numlength: ".$numlength;exit;
                    if($numlength == 4 || $numlength == 5):
                        $number = $estimatetotal / 1000;
                        $number2 = $proctotal / 1000;
                        $numstr = "";
                    elseif($numlength == 6 || $numlength == 7):
                        $number = $estimatetotal / 100000;
                        $number2 = $proctotal / 100000;
                        $numstr = "lac";
                    elseif($numlength == 8 || $numlength == 9):
                        $number = $estimatetotal / 10000000;
                        $number2 = $proctotal / 10000000;
                        $numstr = "cr";
                    else:
                    ...
                    $remaining = $number - $number2;

                    $datarows_materials.= " ['Estimated', ".round($number,2).", ".round($number,2)."],
                                            ['Procured', ".round($number2,2).", ".round($number2,2)."],
                                            ['Remaining', ".round($remaining,2).", ".round($remaining,2)."], ";
        ...
        //echo $datarows_materials; exit;
        $arr = array('result'=>$datarows_materials,'numstr'=>$cashin,'error'=>'No');
        // echo $arr;
        return json_encode($arr);
    }

    public function actionProcurementresourcechart()
    {
        $connection = Yii::$app->db;
        //$projectid=36;
        $uid = Yii::$app->user->Id; 
        ...
        $resourcetotal = $resourceavgrate * $resourceqty;

        /* Resource estimate total end */
        ...
        /* To be procured start */

        $proc_avgrate = $rate;

        if($totrepqty!=0){
            $proc_avgconsupmtn = $activityusage['usedqnty'] / $totrepqty;
        }
        else{
            $proc_avgconsupmtn = 0;
        }

        //$tobeprocured = $proc_avgconsupmtn * $proc_avgrate;

        $costperunit = $proc_avgconsupmtn * $proc_avgrate;

        $reprtd_qty = $totrepqty;

        $billedqty = $clintbillqty;

        if($billedqty!=0){
            $actyield = ($reprtd_qty / $billedqty) * 100;
        }
        else{
            $actyield = 0;
        }

        $estimateqty = $resourceqty;

        if($costperunit!=0){
            $tobeprocured = $costperunit * $actyield * $estimateqty;
        }
        else{
            $tobeprocured = $estimateqty;
        }

        $remaining = $tobeprocured - $procuredtotal;

        //echo $totrepqty; exit;

        /* To be procured end */

        $roundresourcetotal = round($resourcetotal);

        $numlength = strlen((string)$roundresourcetotal);
        //echo "numlength: ".$numlength;exit;
        if($numlength == 4 || $numlength == 5):
            $est_amt = $resourcetotal / 1000;
            $procured_amt = $procuredtotal / 1000;
            $tobeprocured_amt = $tobeprocured / 1000;
            $stocked_amt = $stocktotal / 1000;
            $numstr = "";
        elseif($numlength == 6 || $numlength == 7):
            $est_amt = $resourcetotal / 100000;
            $procured_amt = $procuredtotal / 100000;
            $tobeprocured_amt = $tobeprocured / 100000;
            $stocked_amt = $stocktotal / 100000;
            $numstr = "lac";
        elseif($numlength == 8 || $numlength == 9):
            $est_amt = $resourcetotal / 10000000;
            $procured_amt = $procuredtotal / 10000000;
            $tobeprocured_amt = $tobeprocured / 10000000;
            $stocked_amt = $stocktotal / 10000000;
            $numstr = "cr";
        else:
            $est_amt = $resourcetotal;
            $procured_amt = $procuredtotal;
            $tobeprocured_amt = $tobeprocured;
            $stocked_amt = $stocktotal;
            $numstr = "";
        endif;

        $remaining = $tobeprocured_amt - $procured_amt;

        $color1 = 'blue';

        if($procured_amt>$tobeprocured_amt){
            $tobeproc1 = $tobeprocured_amt;
            $tobeproc2 = 0;
            $color2 = 'green';
            $color3 = 'green';
        }
        else{
            $tobeproc1 = $procured_amt;
            $tobeproc2 = $remaining;
            $color2 = 'blue';
            $color3 = 'red';
        }
        ...
        $datarows_materials.= " 
                            ['Estimated', ".round($est_amt,2).", '".$color1."', 0, '".$color1."', ".round($est_amt,2)."],
                            ['Actual', ".round($tobeproc1,2).", '".$color2."', ".round($tobeproc2,2).", '".$color3."', ".round($tobeprocured_amt,2)."],
                            ['Procured', ".round($procured_amt,2).", '".$color1."', 0, '".$color1."', ".round($procured_amt,2)."],
                            ['Remaining', ".round($remaining,2).", '".$color1."', 0, '".$color1."', ".round($remaining,2)."],
                            ['Stock', ".round($stocked_amt,2).", '".$color1."', 0, '".$color1."', ".round($stocked_amt,2)."], ";
    ...
    public function actionDashbalancesheetreport()
    {
                ...
                foreach($bsitems AS $key=>$bsitem):
                    ...
                    if(count($accounts)>0):
                        foreach($accounts AS $accounthead):
                            ...
                            /*$sql1 = 'INSERT INTO tableau_balancesheet_account (account_sub_id, account_head_id, account_sub_name,account_head_name)
                            VALUES
                            ("'.$accnt_sub->id.'","'.$accnt_head->id.'","'.$accnt_sub->name.'","'.$accnt_head->name.'")';
                            //echo $sql1;exit;
                            $command=$connection->createCommand($sql1);
                            $dataReader=$command->query();*/
        ...
        /*$datarows.="<tr><td style='text-align: center;font-weight: bold;'>Balance Sheet Total</td>
            <td></td><td></td><td></td><td style='text-align: right;'>".number_format((float)abs($projexptot), 2)."</td></tr>";*/
        echo $datarows; exit;
