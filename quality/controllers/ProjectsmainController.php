...
class ProjectsmainController extends Controller
{
    ...
    public function actionProjectperformance()
    {
        ...
        if($row->role_id != 0)
        {
            ...
            foreach ($dataProvider AS $key => $data):
                ...
                //if($projSchedData[0]['start_date'] && $projSchedData[0]['end_date']){
                if($data['start_date'] && $data['end_date']){
                    $projDuration .= '<span style="font-size:11px;"> ('.date('jS M Y', strtotime($data['start_date'])).' - '.date('jS M Y', strtotime($data['end_date'])).')</span>';
                }
                                    ...
                                    <div class="project-info">
                                        ...
                                        <div class="col-md-8">
                                            <div class="type project-spec text-left">
                                                ...
                                                <span class="project-spec-text project-spec-dates"> '.date('jS M Y', strtotime($data['start_date'])).' to '.date('jS M Y', strtotime($data['end_date'])).'</span>
                                            </div>
                                        </div>
	...
	public function actionListscheduleactivities()
    {
        ...
        if($projuser){
            ...
            $sql = "SELECT a.reference_activityid,a.id,a.name,a.actual_start_date,a.actual_end_date,
                        ...
                        a.projectId,a.quantity as sch_quantity, a.assigned_to 
            ...
            $datarows='';
            $datarows='<div class="col-md-12" style="padding:0;">
                            <input type="hidden" id="holiday_arr" value="'.$holiday_dates.'">
                            <input type="hidden" id="holiday_week_arr" value="'.$holiday_weeks.'">
                            <div class="row sacthds">
                                <div class="col-md-1" style="padding-left: 0px;margin-left: 10px;">
                                    <label>#</label>
                                </div>
                                <div class="col-md-3"  style="padding-left: 10px;margin-left: -12px;">
                                    <label>Activity Name</label>
                                </div>
                                <div class="col-md-1 text-center" style="padding-left: 10px;">
                                    <label>Unit</label>
                                </div>
                                <div class="col-md-1 text-center" style="padding-left: 10px;">
                                ...
                                <div class="col-md-1 text-center" style="padding-left: 10px;">
                                    <label title="Duration in Days">Duration</label>
                                </div>
                                <div class="col-md-1 text-center" style="padding-left: 10px;">
                                    <label>Start Date</label>
                                </div>
                ...
                $duration_visible = 'readonly';
                  ...
                  elseif(empty($workgroupact) && $data['reference_activityid']==0){*/
                    ...
                    if($cycletime['duration']!=0){
                        ...
                        if($task['Cycles']!=0 && $task['Resourceunits']!=0 ){
                    ...
                    $q_data = PricingEstimateNew::find()->where(['project_Id'=>$projuser->projectid])->andWhere(['activity_Id'=>$data['activity_id']])->andWhere(['pricing_status'=>0])->one();

                    if(!empty($q_data))
                    {
                        $est_qtyy = $q_data->activity_qty;
                    }else
                    {
                        $est_qtyy = 0;
                    }
                    if(!$data['activity_id'])
                        $est_qtyy =  $data['sch_quantity'];
                    
    				$datarows.='<div class="col-md-12 activitiess activitycontent" id="enggactivities'.$data['id'].'"  data-id="'.$data['id'].'"><div style="padding:0px !important;margin-right:0px;" class="row " id="enggactivities'.$data['id'].'"  data-id="'.$data['id'].'">
    								<div class="col-md-1 nums" style="padding-left: 18px;">
    									<span class="number">'.($totalindex).'</span>
    								</div>
    								<div class="col-md-3 type aname">
    									
    									<span id="'.str_replace(' ','',$data['process']).'activityname'.$data['id'].'">'.$data['name'].'</span>
    									<input class="form-control editenggactivityname" style="display:none;" type="text" id="edit'.str_replace(' ','',$data['process']).'activityname'.$data['id'].'" value="'.$data['name'].'">
    									<span class="error"></span>
    								</div>
    								<div class="col-md-1 type utss text-center">
    									
    									<span id="'.str_replace(' ','',$data['process']).'activityunit'.$data['id'].'">'.$data['unit'].'</span>
    									<input class="form-control editenggactivityunit" style="display:none;" type="text" id="edit'.str_replace(' ','',$data['process']).'activityunit'.$data['id'].'" value="'.$data['unit'].'">
    									<span class="error"></span>
    								</div>
    								<div class="col-md-1 type qtyssss text-center">
    									
    									<span id="'.str_replace(' ','',$data['process']).'activityquantity'.$data['id'].'">'.$est_qtyy.'</span>
    									<input type="hidden" id="cumulatedqty'.$data['id'].'" value="'.$cumulatedQty.'">
    									<input class="form-control editenggactivityquantity" style="display:none;" type="text" id="edit'.str_replace(' ','',$data['process']).'activityquantity'.$data['id'].'" value="'.$est_qtyy.'">
    									<span class="error"></span>
    								</div>
    								<div class="col-md-1 type drtn text-center">
    									
    									<span id="'.str_replace(' ','',$data['process']).'activityduration'.$data['id'].'">'.$data['old_duration'].'</span>
    									<input class="form-control editenggactivityduration" style="display:none;" data-id="'.$data['id'].'" type="text" id="edit'.str_replace(' ','',$data['process']).'activityduration'.$data['id'].'" value="'.$data['old_duration'].'" '.$duration_visible.' '.$task_created.'>
    									<span class="error"></span>
    								</div>
                                    <div class="col-md-1 type srdte text-center">
                                        
                                        <span id="'.str_replace(' ','',$data['process']).'activitystartdate'.$data['id'].'" style="/*font-size:15px;*/">'.$start_date.'</span>
                                        <input class="form-control editactivitystartdate holidayAppliedDatepicker" style="display:none;" data-id="'.$data['id'].'" type="text" id="edit'.str_replace(' ','',$data['process']).'activitystartdate'.$data['id'].'" value="'.$start_date_edit.'" '.$start_date_visible.' readonly>
                                        
                                        <span class="error"></span>
                                    </div>
                                    <div class="col-md-1 type text-center">
                                       
                                        <span class="enddatevertical" id="'.str_replace(' ','',$data['process']).'activityenddate'.$data['id'].'" style="/*font-size:15px;*/">'.$end_date.'</span>
                                    ...
                                    <div class="col-md-3 icon-groups" style="bottom: 3px;">
    									...
    									<button style="width: unset !important;" type="button" class="btn btn-primary viewwbsestimatetasks tstbtnn" data-p="'.$projuser->projectid.'" data-v="' . $data['id'] . '" 
    ...
    public function actionWbsestimatetasklists() 
    {
        ...
        if($projuser){
            ...
            $sql = "SELECT Name AS task,task_Id AS Id FROM tasks_new WHERE activity_Id='".$activityid."' AND status=0";
            // echo $sql;exit;
            $command = $connection->createCommand($sql);
            $dataReader = $command->query();
            ...
            $form = '<style>.table > thead > tr > th, .table > tbody > tr > th, .table > tfoot > tr > th, .table > thead > tr > td, .table > tbody > tr > td, .table > tfoot > tr > td {padding:8px;}</style>';

            $form .= '<form method="POST" action="" id="scheduleform" class="col-md-12 scheduleform">
                       <table cellpadding="0" cellspacing="0 " id="activitytasktable" class="table"> 
                            <thead>
                            <tr>
                                <td colspan="4" class="actheadstyle"><b>Activity</b> : ' . $name . '</td>
                                <!--<td><b>Cycle Unit</b> : <input type="text" class="form-control" id="activityunit" name="activityunit" value="' . $unit . '"></td>
                                <td><b>Cycle Quantity</b> : <input type="text" class="form-control" id="activityqty" name="activityqty" value="' . $quantity . '" ></td>
                                <td></td>-->
                            </tr>
                            ...
                            <tr class="activitylist">
                                ...
                                <td style="width: 80%; padding:13px 25px;" class="taskleftline taskrightline" colspan="3">
                                    <table style="width:100%;">
                                        <tr>
                                            <td style="width:35%">
                                                <b>Report Type : </b>
                                                <div class="radio-inline" style="padding-left:25px;">
                                                    <label style="font-weight:normal;" title="Fixed Quantity"><input type="radio" class="cycle_type" name="cycle_type" value="fixed_qty"  '.$sch_cycle_fixed_qty.' '.$disabledForm.'>Fixed Qty</label>
                                                </div>
                                                <div class="radio-inline">
                                                    <label style="font-weight:normal;" title="Variable Quantity"><input type="radio" class="cycle_type" name="cycle_type" value="variable_qty" '.$sch_cycle_var_qty.' '.$disabledForm.'>Variable Qty </label>
                                                </div>
                                            </td>
                                            ...
                                            <td style="width:35%;text-align: center;">
                                                <b>Report Method : </b>
                                                <div class="radio-inline" style="padding-left:25px;">
                                                    <label style="font-weight:normal;"><input type="radio" class="report_method" name="report_method" value="daily"  '.$sch_rep_mothod_daily.' >Daily</label>
                                                </div>
                                                <div class="radio-inline">
                                                    <label style="font-weight:normal;"><input type="radio" class="report_method" name="report_method" value="weekly" '.$sch_rep_mothod_weekly.' >Weekly </label>
                                                </div>                                            
                                            </td>

                                            <td style="width:30%; text-align: right;">
                                                <b>Estimated Quantity : ' .$est_qty.'</b> '.$est_unit.'
                                                <input type="hidden" id="est_qty" value="'.$est_qty.'">
                                            </td>
                                        </tr>
                                    </table>
                            ...
                            <tr>
                                <td style="width: 10%;" class="taskblankrow"></td>
                                <td style="width: 35%; padding:10px 15px; background-color:#f4f4f4; font-size:13px; padding-left:25px;" class="taskleftline ">
                                    <b>Task</b>
                                </td>
                                <td style="width: 35%; background-color:#f4f4f4;  font-size:13px; padding-left:20px;" class="">
                                    <b>Duration</b>(Hours)
                                </td>
                                <td style="width: 10%; background-color:#f4f4f4;" class=""></td>
                                <td style="width: 10%;" class="taskblankrow">
                                    <input type="hidden" name="activityid" value="'.$activityid.'">';
                                    if($wbsactivity):
                                    $form .= '<input type="hidden" name="wbsid" value="'.$wbsactivity['wbs_id'].'">
                                    <input type="hidden" name="estimateid" value="'.$wbsactivity['id'].'">';
                                    endif;
            ...
            if(count($tasks)>0){
                foreach ($tasks AS $key => $task):
                    ...
                    //$sqlqr = "SELECT Budgeted_Duration,End_Duration,status FROM schedule_task_new WHERE activity_Id='".$activityid."' AND activity_name='".$name."' AND task_Id='".$task['Id']."'";
                    $sqlqr = "SELECT Budgeted_Duration,End_Duration,status FROM schedule_task_new WHERE activity_Id='".$activityid."'  AND task_Id='".$task['Id']."'";
                    ...
                    if(count($report)!=0){
                        ...
                        foreach($report AS $reportid){
                            ...
                            //get actualduration from each cycle to find the average duration of reported tasks
                            ...
                            $actualduration = $actdur['actualduration'];
                        ...
                        if(!empty($sche_tasks)):
                            $averageactulal =  $sche_tasks['Budgeted_Duration'];
                        else:
                            $averageactulal =  0;
                        endif;
                    }
                    ...
                    /*$form .= '<tr id="activityrow' . $task['Id'] . '">
                                <td><span id="taskname'.$task['Id'].'" data-id="'.$task['Id'].'">' . $task['task'] . '</span>
                                <input type="text" class="form-control taskname_edit" id="taskname_edit'.$task['Id'].'"  data-id="' . $task['Id'] . '" name="taskname" value="' . $task['task'] . '" style="display:none;">
                                </td>
                                <td id="b">
                                <input type="hidden" name="task_id[]" value="'. $task['Id'] .'">
                                <input type="text" class="form-control bduration" id="bduration' . $key . '"  data-id="' . $key . '" name="bduration[]" value="' . $sche_tasks['Budgeted_Duration'] . '"><span class="error"></span></td>
                                <!--<td><input type="text" class="form-control eduration"  readonly="readonly" id="eduratio' . $key . '"  data-id="' . $key . '" name="eduration[]" value=""><span class="error"></span></td>-->
                                <td class="icon-groups">
                                <!--<input type="checkbox" ' . ($sche_tasks['status'] == '1' ? 'checked' : '') . ' name="cyclexclude' . $task['Id'] . '" class="activity" data-id="' . $task['Id'] . '" value="' . $task['Id'] . '" style="visibility: visible;">-->
                                <a class="btn btn-primary icon-pencil edittaskname" id="edittaskname'.$task['Id'].'" data-id="'.$task['Id'].'"></a>
                                <a class="btn btn-primary icon-save savetaskname" id="savetaskname'.$task['Id'].'" data-id="'.$task['Id'].'" style="display:none;"></a>
                                <a class="btn btn-danger icon-trash1" id="removetask" data-id="'.$task['Id'].'"></a>
                                </td>
                                </tr>';*/
                    $buttonrow = '';
                    if($user->account_type != 'perfm_pad_reporting_only'){
                        if($key==0){
                            //$buttonrow = '<div class="icon-groups" style="float:left;"><a class="btn btn-primary icon-plus icon_plus" id="tasknewrow"  title="Add more Task"></a><a class="btn btn-danger icon-times taskremoverow" data-id="'.$key.'" id="taskremoverow'.$key.'" title="Remove Task"></a></div>';
                            $buttonrow = '<div class="icon-groups" style="justify-content: center;">
                                                <a class="btn btn-primary icon-plus icon_plus" id="tasknewrow"  title="Add more Task"></a>
                                            </div>';

                        }
                        else{
                            $buttonrow = '<div class="icon-groups" style="justify-content: center;"><a class="btn btn-danger icon-times taskremoverow" data-id="'.$key.'" id="taskremoverow'.$key.'" title="Remove Task"></a></div>';
                        }
                    }

                    $form .= '  <tr class="tastrow" id="activityrow' . $key . '">
                                <td style="width: 10%;" class="taskblankrow"></td>
                                <td style="width: 35%; padding: 10px 20px;" class="taskleftline ">
                                    <input type="hidden" name="tasknewid[]" value="' . $task['Id'] . '">
                                    <input type="text" class="form-control taskname_edit" name="taskname[]" value="' . $task['task'] . '"  '.$disabledForm.'>
                                </td>
                                <td style="width: 35%;  padding: 10px 20px;" class="">
                                    <input type="number" class="form-control taskduration_edit" name="taskduration[]" value="'.$sche_tasks['Budgeted_Duration'].'"  '.$disabledForm.'>
                ...
                $form .= '  <tr class="tastrow" id="activityrow0">
                                <td style="width: 10%;" class="taskblankrow"></td>
                                <td style="width: 35%;" class="taskleftline">
                                    <input type="hidden" name="tasknewid[]" value="">
                                    <input type="text" class="form-control taskname_edit" name="taskname[]" value="">
                                </td>
                                <td style="width: 35%;">
                                    <input type="number" class="form-control taskduration_edit" name="taskduration[]" value="">
                                </td>
                                <td style="width: 10%;" class="taskrightline">
                                    <a class="btn btn-primary icon-plus icon_plus" id="tasknewrow"></a>
                                </td>
                                <td style="width: 10%;" class="taskblankrow"></td>
                            </tr>';
            }

            $form .= '<tr class="activitylist">
                    <td style="width: 10%;" class="taskblankrow"></td>
                    <td style="width: 35%; padding-bottom:10px; padding-top:10px; padding-right: 20px; text-align:right; background-color: #f4f4f4;" class="taskleftline "><b>Cycle Time</b></td>

                    <td  style="width: 35%; padding-bottom:10px; padding-top:10px; padding-left: 25px !important; background-color: #f4f4f4;" class="cycletimestyle ">
                        <span id="cyclebud" style="font-weight:bold;">' . $btime . '</span> Hours <input type="hidden" name="totalbudget" id="budcycle" value="' . $btime . '">
                    </td>
                    <!--<td  style="width: 25%; "><span id="cycleact">' . $atime . '</span> <input type="hidden" name="aduration" id="actcycle" value="' . $atime . '"></td>-->
                    <td style="width: 10%; background-color: #f4f4f4;" class="taskrightline"></td>
                    <td style="width: 10%;" class="taskblankrow"></td>
                    </tr>';
            ...
            $form .= '  <tr>
                                <td style="width: 10%;" class="taskblankrow"></td>
                                <td style="width: 80%; padding: 15px 15px 10px 15px;" colspan="3" class="taskleftline taskrightline">
                                    <table width="100%">
                                        <tr>
                                            <td style="width: 20%; text-align:center; padding-right:25px;"><b>Cycle Qty</b> ('.trim($est_unit).')</td>
                                            <td style="width: 20%; text-align:center; padding-right:25px;"><b>No. of Cycles</b></td>
                                            <td style="width: 20%; text-align:center; padding-right:25px;" ><b>Working Hours</b></td>
                                            <td style="width: 20%; text-align:center; padding-right:25px;"><b>No. of Resource units</b></td>
                                            <td style="width: 20%; text-align:center;"><b>No. of Units per Cycle</b></td>
                                        </tr>
                                    </table>
                                </td>
                                <td style="width: 10%;" class="taskblankrow"></td>
            ...
            $form .= '  <tr>
                                <td style="width: 10%;" class="taskblankrow"></td>
                                <td style="width: 80%;  border-bottom: 1px solid #ddd; padding: 15px;" colspan="3" class="taskleftline taskrightline">
                                    <table width="100%">
                                        <tr>
                                            <td style="width: 20%; padding-right:25px;" >
                                                    <input type="number" class="form-control" name="cycle_qty" id="cycle_qty" value="' . $sch_cyc_qty . '"  '.$disabledForm.'><span class="error"></span>
                                                    <!--<input type="hidden" name="wrkhrs" value="24"> -->
                                            </td>
                                            <td style="width: 20%;  padding-right:25px;" >
                                                    <input type="number" class="form-control" name="cycles" id="no_of_cycles" value="' . $sch_cyc . '" readonly><span class="error"></span>
                                            </td>
                                            <td style="width: 20%; padding-right:25px;" >
                                                <select class="form-control" name="wrkhrs" id="wrkkhrs"  '.$disabledForm.'>
                                                    <option value="0">Select Hours</option>
                                                    <option value="8" '. (($sch_work == '8') ? 'selected' : '').'>8</option>
                                                    <option value="10" '. (($sch_work == '10') ? 'selected' : '').'>10</option>
                                                    <option value="12" '. (($sch_work == '12') ? 'selected' : '').'>12</option>
                                                    <option value="24" '. (($sch_work == '24') ? 'selected' : '').'>24</option>
                                                </select>
                                                <span class="error"></span>
                                            </td>
                                            <td style="width: 20%; padding-right:25px;">';
                                            if($wbsactivity):
                                                $form .= '<input type="hidden" name="iowid" value="' . $wbsactivity['wbs_id'] . '"> ';
                                                    ...
                                                    $form .= '<input style="" type="number" class="form-control" name="resourceunit" id="resourceunit" value="' . $sch_res . '"  '.$disabledForm.'><span class="error"></span>';

                            $form .= '      </td>
                                            <td style="width: 10%; padding-right:5px;" >
                                                    <input type="number" class="form-control" name="cycle_units" id="cycle_units" value="' . $sch_cycle_units . '" '.$sch_cycle_unit_style.'  '.$disabledForm.'><span class="error"></span>
                                            </td>

                                            <td style="width:  10%;" >
                                                    <input type="text" placeholder="Unit" class="form-control" name="cycle_unit_type" id="cycle_unit_type" value="' . $sch_cycle_unit_type . '" '.$sch_cycle_unit_style.'  '.$disabledForm.'><span class="error"></span>
                                            </td>
            ...
            /*$form .= '  <tr>
                            <td style="width: 10%;" class="taskblankrow"></td>
                            <td colspan="2" class="taskleftline">
                                <table>
                                    ...
                                    <tbody>
                                    <td style="width: 20%;"><b>Number of Resource units</b></td>
                                    <td style="width: 20%;"><b>Number of Cycles</b></td>
                                    </tbody>
                                </table>
                            
                            </td>
                            
                            <td style="width: 10%;" class="taskrightline"></td>
                            <td style="width: 10%;" class="taskblankrow"></td>
                        </tr>
                        <tr>
                            <td style="width: 10%;" class="tasksaveblankrow"></td>
                            <td colspan="2" class="taskleftline" style="border-bottom: 1px solid #ddd;">
                                <table>
                                    <thead></thead>
                                    <tbody>
                                    <td style="width: 20%;">';
                                    ...
                                    }else
                                    {
                                        ...
                                        $form .= '<input style="width:95%" type="text" class="form-control" name="resourceunit" id="resourceunit" value="' . $sch_res . '"><span class="error"></span>
                                        </td>
                                        <td style="width: 20%;">
                                            <input style="width:95%" type="text" class="form-control" name="cycles" id="cycles" value="' . $sch_cyc . '"><span class="error"></span>
                                            <input type="hidden" name="wrkhrs" value="24"> 
                                        </td>
                                        ';
                                        <td style="width: 20%;"><select class="form-control" name="wrkhrs" id="wrkkhrs">';
                                   
                                        if($sch_work == '8')
                                        {
                                            $form .= '<option value="0">Select Hours</option>
                                        ...
                                        }elseif($sch_work == '24')
                                        {
                                            ...
                                            $form .= '<option value="0" selected="selected">Select Hours</option>
                                            <option value="8">8</option>
                                            <option value="10">10</option>
                                            <option value="12">12</option>
                                            <option value="24">24</option>';
                            ...
                            $form .= '</select><span class="error"></span>
                            ...
                            <td style="width: 10%;border-bottom: 1px solid #ddd;" class="taskrightline"></td>
                            <td style="width: 10%;" class="taskblankrow"></td>
                        </tr>';*/
            ...
            $form .= '</tbody>
                    ...
                    <div class="text-center">
                        <span class="error" id="taskScheduleError"></span>
                    </div>
                    <div class="text-center">
    ...
    public function actionSchedulecreate()
    {
        ...
        if($count!=0):
            ...
            for ($i = 0; $i < $count; $i++) {
                ...
                if($_POST['tasknewid'][$i]=='' && $_POST['taskname'][$i]!=''){

                    $model = New TasksNew();
                    $model->activity_Id = $actid;
                    $model->Name = $_POST['taskname'][$i];
                    if($wrk->process==""){
                        $model->process_id = 0;
                    }else{
                ...
                //else{
                elseif($_POST['tasknewid'][$i]!=''){
                    $tasknew=TasksNew::findOne($_POST['tasknewid'][$i]); 
                    $tasknew->Name = $_POST['taskname'][$i];
                    $tasknew->status = 0;
                    $tasknew->save(false);

                    $taskid = $_POST['tasknewid'][$i];
                }
                ...
                if($taskid!=0){
                    ...
                    $sql1 = 'INSERT INTO schedule_task_new ( wbs_id, activity_Id, process_Id,wbs_activity_Id, task_Id, Budgeted_Duration, End_Duration, status,activity_name ) 
                        VALUES
                        ( "0","'.$actid.'","0","0","'.$taskid.'","'.$_POST['taskduration'][$i].'","0","0","'.$activityname.'")';
                    //echo $sql1;exit;
                    $command=$connection->createCommand($sql1);
                    $dataReader=$command->query();
        ...
        if($_POST['resourceunit']==''){
            $_POST['resourceunit'] = 0;
        }
        ...
        if($_POST['totalbudget']!=0 && $_POST['totalbudget']!=''){

            //$duration = ($_POST['totalbudget'] / $_POST['resourceunit']) * $_POST['cycles'];
            //$wrk->duration= $duration;
            $durations = ($_POST['totalbudget'] * $_POST['cycles']) / $_POST['wrkhrs'];
            $duration = $durations / $_POST['resourceunit'];

            $roundedduration = round($duration);
            $wrk->old_duration= $roundedduration;
            $dateadjust = $roundedduration - 1;
            //$wrk->end_date = date('Y-m-d', strtotime($wbsact->start_date. ' + '.$dateadjust.' days')); 

            $sch_actual_end_date  = date('Y-m-d', strtotime($wrk->actual_start_date. ' + '.$dateadjust.' days'));

            $getDateAfterHoliday  = Yii::$app->helper->getDateAfterHoliday($wrk->actual_start_date, $wrk->projectId, $roundedduration);
            $wrk->actual_end_date = date('Y-m-d', strtotime($getDateAfterHoliday));
            $wrk->save(false);
        }
	...
	public function actionScheduleactivityupdate()
    {
      ...
      /*  if($_POST['lag']!=''){
        ...
        //$availableActivites = ActivityRelations::model()->findAll(array('condition'=>'projectId ='.$activity1->projectId.' AND status =0 AND wbs_schedule_status=0','order'=>'precedent_schedule_item ASC'));
        
        endif;
        
        
        $arr = array('Id' => $_POST['id'],'Name'=>$_POST['name'],'Unit'=>$_POST['unit'],'Duration'=>$activity1->duration,'Startdate'=>date("d-m-Y", strtotime($activity1->start_date)),'Enddate'=>date("d-m-Y", strtotime($activity1->end_date)),'Editstartdate'=>date("Y-m-d", strtotime($activity1->start_date)),'Editenddate'=>date("Y-m-d", strtotime($activity1->end_date)),'Quantity'=>$_POST['quantity'],'Lag'=>$_POST['lag'],'error'=>'No');
        return json_encode($arr);
	}
   ...
   public function GetRelationcorrectfinal($projectid)
    ...
    public function actionActivityrelation()
    {
        ...
        if($projuser)
        {
            ...
            foreach($scheduleItems as $key => $data) {
                $selected = "";
                if($data['scheduleitem_id'] == $_POST['filter_schedule_item'])
                    $selected = "selected";
            ...
            foreach($scheduleactivities as $key => $data) {
                $selected = "";
                if($data['id'] == $_POST['filter_schedule_activity'])
                    $selected = "selected";
            ...
            if($_POST['Relactionename'] != ''){
                ...
                if($filter_schedule_item = $_POST['filter_schedule_item']){
                    $relations->andWhere(['or',['precedent_schedule_item' => $filter_schedule_item], ['dependent_schedule_item' => $filter_schedule_item]]);
                }
                if($filter_schedule_activity = $_POST['filter_schedule_activity']){
                    $relations->andWhere(['or',['precedent_activity' => $filter_schedule_activity], ['dependent_activity' => $filter_schedule_activity]]);
                }
                        ...
                        foreach($relations AS $key => $relation)
                        {
                            ...
                            $reported = ProgressReport::find()->where(['activity_id' => $relation['dependent_activity']])->andWhere(['status' => 1])->all();
                               ...
                               <div class="col-md-1 icon-groups">				
                                   ...
                                   <a class="btn btn-primary icon-pencil editrelation" value="'.$relation['id'].'" id="editrelationbutton'.$relation['id'].'" title="Edit Relation" href="#"></a>
                                   <a class="btn btn-primary icon-save saveeditrelation" value="'.$relation['id'].'" id="saveeditrelationbutton'.$relation['id'].'" title="Save" style="display:none;" href="#"></a>
                                   <a class="btn btn-danger  icon-trash1 deleterelation" value="'.$relation['id'].'"  id="deleterelationbutton'.$relation['id'].'" title="Delete Relation" '.$relationstyle.' href="#"></a>
                               </div>
                           </div>';
            ...
            if(count($relations) > 0)
            {
                ...
                foreach($relations AS $key => $relation)
                {
                    ...
                    $html .= '<tr id="relationrow'.$relation['id'].'">
                                ...
                                <td><button type="button" class="btn btn-primary deleterelation" value="'.$relation['id'].'"  id="deleterelationbutton'.$relation['id'].'" title="Delete Relation">
    ...
    public function actionDeleterelation()
    {
        ...
        if($model->save(false)){
           ...
           $schedulereport = ScheduleProgressReport::find()->where(['activity_id' => $model->dependent_activity])->one();
           if($schedulereport){

               $dependentAct = Scheduleactivities::findOne($model->dependent_activity);
               $prevduration = $dependentAct->duration - 1;
               $dependentAct->start_date = $dependentAct->start_date;
               $dependentAct->end_date = date('Y-m-d', strtotime($dependentAct->start_date. ' + '.$prevduration.' days'));
               $dependentAct->save(false);
               ...
               //-- DATES CHANGED AS PER FIRST SCHEDULE-----
               $prevdatediff = strtotime($dependentAct->actual_end_date) - strtotime($dependentAct->actual_start_date);
               $prevduration = round($prevdatediff / (60 * 60 * 24)) + 1;
               $dependentAct->start_date = $dependentAct->actual_start_date;
               $dependentAct->end_date = $dependentAct->actual_end_date; 
               $dependentAct->duration = $prevduration;
               $dependentAct->old_duration = $prevduration;
               //--------------------------

               /*
               //-- DATES CHANGED TO AS OF TODAY-----
               $prevduration = $dependentAct->old_duration;
               $dependentAct->start_date = date('Y-m-d');
               $dependentAct->end_date = date('Y-m-d', strtotime($dependentAct->start_date. ' + '.$prevduration.' days'));
               $dependentAct->actual_start_date = date('Y-m-d');
               $dependentAct->actual_end_date = date('Y-m-d', strtotime($dependentAct->start_date. ' + '.$prevduration.' days'));
               $dependentAct->act_start_date = date('Y-m-d');
               $dependentAct->duration = $prevduration;
               //--------------------------
                */
                       ...
                       $datarows.="<tr id='tempprodrow' class='vendor-column'>
                        ...
                        foreach($wbsactivities as $key=> $wbsactivity):
                            ...
                            $datarows.='<tr  id="Estimateiowactivitiesrow' . $wbsactivity['id'] . '" data-id="' . $wbsactivity['id'] . '">
                                        ...
                                        <span class="error"></span></td>
                                        <td style="text-align: right"><span id="'.str_replace(' ','',$processname).'amount' . $wbsactivity['id'] . '">' . number_format((float)$amount,2) . '</span>
        ...
        if (isset($_POST['itemid'])) {
            ...
            if($estimatecount > 0):
                for ($i = 0; $i < count($_POST['itemid']); $i++) {
                    ...
                    $sqll        = "UPDATE scheduleactivities SET quantity='".$_POST['quantity'][$i]."' WHERE activity_id='".$itemid."' ";
                    $command     = $connection->createCommand($sqll);
                    $dataReaderr = $command->query();
                    ...
                    if($estimateactdat):
                        ...
                        //---- Update No of Cycles ---------------------
                        if($estimateactdat['activity_qty'] != $_POST['quantity'][$i]){

                            $sch_act        = Scheduleactivities::find()
                                                ->where(['projectId'=>$_POST['Project_Id']])
                                                ->andWhere(['activity_id'=>$itemid])
                                                ->one();

                            $tot_task_dur   = Yii::$app->helper->getDataFromQuery(" SELECT SUM(Budgeted_Duration) AS totalqty 
                                                            FROM schedule_task_new 
                                                            WHERE  activity_Id=".$sch_act->id
                                                )['totalqty'];


                            if($tot_task_dur > 0){
                                $sch_act_new    = ScheduleActivityNew::find()->where(['Project_Id'=>$_POST['Project_Id']])
                                                    ->andWhere(['actvity_id'=>$sch_act->id])
                                                    ->one();
                                //$sch_act_new->cycle_qty = 
                                $sch_act_new->Cycles = ($_POST['quantity'][$i] / $sch_act_new->cycle_qty);
                                $sch_act_new->save(false);


                                $durations = ($tot_task_dur * $sch_act_new->Cycles) / $sch_act_new->Workhours;
                                $duration = round($durations / $sch_act_new->Resourceunits);
                                $sch_act->old_duration= $duration;
                                $sch_act->save(false);
                            }
                        }
                        //--------------------------------------------------
   ...
   public function actionRemoveitemold()
   {
    ...
    public function actionNewganttchart($id, $layout = 'true')
    {
        ...
        //$this->GetRelationcorrect($id);

        $connection = \Yii::$app->db; 
        $query = "SELECT * FROM wbsscheduleitems WHERE projectId='" . $id . "' AND status=0";
        $command = $connection->createCommand($query);
        $dataReader = $command->query();
        $items = $dataReader->readAll();
        $html = '';
        ...
        foreach($items as $item){
            ...
            foreach($tasks as $task){
               if($task['reference_activityid']!=0):
                  $Wbsscheduleitems=Wbsscheduleitems::find()->Where(['scheduleitem_id' => $item['scheduleitem_id']])->andWhere(['status'=>0])->one();
                  $workgroupact = WorkgroupActivitiesNew::find()->Where(['id' => $task['reference_activityid']])->andWhere(['wbs_id'=>$Wbsscheduleitems->reference_itemid])->andWhere(['schedule'=>1])->andWhere(['pricing_status'=>0])->one();
               endif;
                //if(count($workgroupact)>0){
                if(isset($workgroupact)){
                 if($workgroupact->process_Id==2 || $workgroupact->process_Id==5):
                   $progressreport=ProgressReport::find()->Where(['activity_id' => $task['id']])->andWhere(['status'=>0])->one();
                    if($progressreport){
                        $visible = '';   
                        $progressstart_date = $progressreport->start_date;
                    }
                    else{
                        $visible = 'style="display:none;"';  
                        $progressstart_date = '';
                    ...
                    $Relations = ActivityRelations::find()->Where(['precedent_activity' => $task['id']])->andWhere(['precedent_schedule_item'=>$task['scheduleitem_id']])->andWhere(['status'=>0])->all();

                    $Relations1 = ActivityRelations::find()->Where(['dependent_activity' => $task['id']])->andWhere(['dependent_schedule_item'=>$task['scheduleitem_id']])->andWhere(['status'=>0])->all();
                    //echo count($Relations); echo count($Relations1); exit;
                    if(count($Relations)!=0 or count($Relations1)!=0){                
                        $html .= '<tr class="ui-state-default  '.$class.'" data-id="'.$task['id'].'">
                                    <td>&nbsp&nbsp'.$task['name'].'</td>
                                    <td><input type="text" class="form-control duration duration_'.$task['id'].'" data-id="duration_'.$task['id'].'" name="duration_'.$task['id'].'" value="'.$task['duration'].'" readonly></td>
                                    <td>
                                    <input type="date" class="form-control date_field_start start_date_'.$task['id'].'" data-id="start_date_'.$task['id'].'" name="start_date_'.$task['id'].'" value="'.$task['start_date'].'" readonly>
                                    ...
                                    <td style="display:none;"><input type="text" class="form-control lag lag_'.$task['id'].'" data-id="lag_'.$task['id'].'" name="lag_'.$task['id'].'" value="'.$task['lag'].'" readonly></td>
                                    <td>
                                    <input type="date" class="form-control actual_start_date_'.$task['id'].'" data-id="actual_start_date_'.$task['id'].'" name="actual_start_date_'.$task['id'].'" value="'.$progressstart_date.'" '.$visible.' readonly>
                                    <span class="error" style="display:none;color:red;font-size: 12px;"></span>
                                    </td>
                                   <!-- <td>
                                    <input type="date" class="form-control actual_end_date_'.$task['id'].'" data-id="actual_end_date_'.$task['id'].'" name="actual_end_date_'.$task['id'].'" value="'.$task['actual_end_date'].'">
                        ...
                        $html .= '<tr class="ui-state-default  '.$class.'" data-id="'.$task['id'].'">
                                    <td>&nbsp&nbsp'.$task['name'].'</td>
                                    <td><input type="text" class="form-control duration duration_'.$task['id'].'" data-id="duration_'.$task['id'].'" name="duration_'.$task['id'].'" value="'.$task['duration'].'"></td>
                                    <td>
                                    <input type="date" class="form-control date_field_start start_date_'.$task['id'].'" data-id="start_date_'.$task['id'].'" name="start_date_'.$task['id'].'" value="'.$task['start_date'].'">
                                    <span class="error" style="display:none;color:red;font-size: 12px;" id="start_date_'.$task['id'].'"></span>
                                    </td>
                                    <td>
                                    <input type="date" class="form-control date_field_end end_date_'.$task['id'].'" data-id="end_date_'.$task['id'].'" name="end_date_'.$task['id'].'" value="'.$task['end_date'].'">
                                    <span class="error" style="display:none;color:red;font-size: 12px;" id="end_date_'.$task['id'].'"></span>
                                    </td>
                                    <td style="display:none;"><input type="text" class="form-control lag lag_'.$task['id'].'" data-id="lag_'.$task['id'].'" name="lag_'.$task['id'].'" value="'.$task['lag'].'" readonly></td>
                                    <td>
                                    <input type="date" class="form-control actual_start_date_'.$task['id'].'" data-id="actual_start_date_'.$task['id'].'" name="actual_start_date_'.$task['id'].'" value="'.$progressstart_date.'" '.$visible.'>
                                    <span class="error" style="display:none;color:red;font-size: 12px;"></span>
                                    </td>
                                   <!-- <td>
                                    <input type="date" class="form-control actual_end_date_'.$task['id'].'" data-id="actual_end_date_'.$task['id'].'" name="actual_end_date_'.$task['id'].'" value="'.$task['actual_end_date'].'">
                ...
                //elseif(count($workgroupact)==0 && $task['reference_activityid']==0){
                //else{
                elseif(empty($workgroupact) && $task['reference_activityid']==0){
                    $progressreport=ProgressReport::find()->Where(['activity_id' => $task['id']])->andWhere(['status'=>0])->one();
                    if($progressreport){
                     $visible = '';  
                     $progressstart_date = $progressreport->start_date;
        ...
        $query = "SELECT * FROM activity_relations WHERE projectId='" . $id . "' AND status=0";
        $command = $connection->createCommand($query);
        $dataReader = $command->query();
        ...
        foreach($relations AS $relation)
        {
            $query = "SELECT * FROM scheduleactivities WHERE scheduleitem_id='" . $relation['precedent_schedule_item'] . "' AND status=0";
            $command = $connection->createCommand($query);
            $dataReader = $command->query();
            $precedentActivities = $dataReader->readAll();

            $query = "SELECT * FROM scheduleactivities WHERE scheduleitem_id='" . $relation['dependent_schedule_item'] . "' AND status=0";
            //$query = "SELECT * FROM scheduleactivities WHERE scheduleitem_id='" . $relation['precedent_schedule_item'] . "' AND status=0";
            $command = $connection->createCommand($query);
            $dataReader = $command->query();
            $dependentActivities = $dataReader->readAll();
             
           $Wbsscheduleitems=Wbsscheduleitems::find()->Where(['scheduleitem_id' => $relation['precedent_schedule_item']])->andWhere(['status'=>0])->one();

           if($Wbsscheduleitems){
           $relationsHtml .= '<tr id="relationrow_'.$relation['id'].'">
           <td><select class="form-control precedent_schedule_item" data-id='.$relation['id'].' id="precedent_schedule_item_'.$relation['id'].'">';
           foreach($scheduleItems AS $item)
           {
               $selected ='';
               if($item['scheduleitem_id'] == $relation['precedent_schedule_item'])
                    $selected = 'selected';
             $relationsHtml .= '<option value="'.$item['scheduleitem_id'].'" '.$selected.'>'.$item['name'].'</option>';
           }


           $relationsHtml .='</select></td>';

           $relationsHtml .= '<td><select class="form-control precedent_activity" data-id='.$relation['id'].' id="precedent_activity_'.$relation['id'].'">';
           $relationsHtml .='<option value="">Select Precedent Activity</option>';
           foreach($precedentActivities AS $activity)
           {
               $selected ='selected';
               /*if($activity['id'] == $relation['precedent_activity'])
                    $selected = 'selected';
                $relationsHtml .= '<option value="'.$activity['id'].'" '.$selected.'>'.$activity['name'].'</option>';*/
               $workgroupact = WorkgroupActivitiesNew::find()->Where(['id' => $activity['reference_activityid']])->andWhere(['wbs_id'=>$Wbsscheduleitems->reference_itemid])->andWhere(['schedule'=>1])->andWhere(['pricing_status'=>0])->one();
               ...
               if($workgroupact){
                  if($activity['id'] == $relation['precedent_activity']){
                    ...
                    $relationsHtml .= '<option value="'.$activity['id'].'">'.$activity['name'].'</option>';
                  }
                }
                //elseif(count($workgroupact)==0 && $activity['reference_activityid']==0 && $activity['id']==$relation['precedent_activity']){
                elseif(empty($workgroupact) && $activity['reference_activityid']==0){
                  if($activity['id']==$relation['precedent_activity']){
                    $relationsHtml .= '<option value="'.$activity['id'].'" '.$selected.'>'.$activity['name'].'</option>';
                  }
                  else{
           ...
           $relationsHtml .= '<td><select class="form-control dependent_schedule_item" data-id='.$relation['id'].' id="dependent_schedule_item_'.$relation['id'].'">';
         //    dependent html 
           foreach($scheduleItems AS $item)
           {
               $selected ='';
               if($item['scheduleitem_id'] == $relation['dependent_schedule_item'])
                    $selected = 'selected';
             $relationsHtml .= '<option value="'.$item['scheduleitem_id'].'" '.$selected.'>'.$item['name'].'</option>';
           }
           ...
           $relationsHtml .= '<td><select class="form-control dependent_activity" data-id='.$relation['id'].' id="dependent_activity_'.$relation['id'].'">';
           $relationsHtml .='<option value="">Select Dependent Activity</option>';
           foreach($dependentActivities AS $activity)
           {
               $selected ='selected';
               /*if($activity['id'] == $relation['dependent_activity'])
                    $selected = 'selected';
                $relationsHtml .= '<option value="'.$activity['id'].'" '.$selected.'>'.$activity['name'].'</option>';*/
                
                $workgroupact = WorkgroupActivitiesNew::find()->Where(['id' => $activity['reference_activityid']])->andWhere(['schedule'=>1])->andWhere(['pricing_status'=>0])->one();
           ...
           if($relation['relation_type'] == 1)
            ...
            if($relation['relation_type'] == 3)
                $ffselected = 'selected'; 
            $activity_id = Scheduleactivities::findOne($relation['dependent_activity']);
            //$reported = ProgressReport::model()->find(array('condition'=>'activity_id ='.$relation['dependent_activity'].' AND status =1'));
            $reported = ScheduleProgressReport::find()->Where(['activity_id' => $relation['dependent_activity']])->andWhere(['!=', 'cumulated_qty', 0])->one();
           ...
           if($relation['relation_type'] == 1 || $relation['relation_type'] == 3 || $relation['relation_type'] == 2){

               $relationsHtml .='</select></td>
               <td><select class="form-control relation_type" data-id="'.$relation['id'].'" value='.$relation['relation_type'].' id="relation_type_'.$relation['id'].'"><option value="1" '.$ssselected.'>SS</option><option value="2" '.$fsselected.'>FS</option><option value="3" '.$ffselected.'>FF</option></select></td>
               <td><input class="form-control activitylag" data-id='.$relation['id'].' style="width:80px;" type="text" id="activitylag_'.$relation['id'].'" value="'.$activity_id['lag'].'"></td>
               <td><button id="save-relation_'.$relation['id'].'" type="button" value="'.$relation['id'].'" class="save-relation" style="position: relative;
                width: 100px;
                margin-bottom: 3px;
                background: #072c47;
                border-radius: 20px;
                ...
                color: #fff;
                font-size: 12px;
                padding: 3px" '.$relationstyle.'>Save</button></td>

                <td><button id="delete-relation_'.$relation['id'].'" type="button" value="'.$relation['id'].'" class="delete-relation" style="position: relative;
                width: 100px;
                margin-bottom: 3px;
                background: #072c47;
                border-radius: 20px;
                ...
                color: #fff;
                font-size: 12px;
                padding: 3px" '.$relationstyle.'>Remove</button></td>';
               
           }
           else{
               $relationsHtml .='</select></td>
               <td><select class="form-control relation_type" data-id="'.$relation['id'].'" value='.$relation['relation_type'].' id="relation_type_'.$relation['id'].'"><option value="1" '.$ssselected.'>SS</option><option value="2" '.$fsselected.'>FS</option><option value="3" '.$ffselected.'>FF</option></select></td>
               <td><input style="display:none;width:80px;" class="form-control activitylag" data-id='.$relation['id'].' type="text" id="activitylag_'.$relation['id'].'" value="'.$activity_id['lag'].'"></td>
               <td><button id="save-relation_'.$relation['id'].'" type="button" value="'.$relation['id'].'" class="save-relation" style="position: relative;
                width: 100px;
                margin-bottom: 3px;
                background: #072c47;
        ...
        // echo $html;exit;
        //$this->layout = 'gantlayout';

        if($layout == 'false') $this->layout = false;

        return $this->render('newganttview', array(
          'activities' => $html,
          'projectId' => $id,
          'projectName' => $project->Name,
          'projectDuration' => $project->duration.' Days',
          'relationsHtml' => $relationsHtml,
        ));

    }

    public function actionCheckdatevalidity()
    {

        // echo $_POST['date'];
        $date = $_POST['date'];
        $dataId = $_POST['dataId'];
        $activityId =  substr($dataId, strrpos($dataId, '_') + 1);
        $fieldName = substr($dataId, 0,strrpos($dataId, '_'));
        $connection = \Yii::$app->db; 
        // dependent item
        $sql = "SELECT * FROM activity_relations WHERE dependent_activity='" .$activityId. "' AND status=0";
        $command = $connection->createCommand($sql);
        $dataReader = $command->query();
        $relation = $dataReader->readAll();
        $array= array();
        if(count($relation) > 0)
        {
            foreach($relation AS $rel)
            {
                ...
                {
                    if($rel['relation_type'] == 1) //SS
                        $array[]=array('id' =>'start_date_'.$rel['precedent_activity'],'date' =>$_POST['date']);
                    elseif($rel['relation_type'] == 2) //SF
                        $array[]=array('id' =>'end_date_'.$rel['precedent_activity'],'date' =>date('Y-m-d', strtotime($_POST['date'] .' -1 day'))); 
                }
            }
        }

         // precedent item
         $sql1 = "SELECT * FROM activity_relations WHERE precedent_activity='" .$activityId. "' AND status=0";
         $command = $connection->createCommand($sql1);
         $dataReader = $command->query();
         $relation1 = $dataReader->readAll();
         
         if(count($relation1) > 0)
         {
            foreach($relation1 AS $rel)
            {
                ...
                {
                    if($rel['relation_type'] == 1) //SS
                        $array[]=array('id' =>'start_date_'.$rel['dependent_activity'],'date' =>$_POST['date']);
                }
                else
                {
                    if($rel['relation_type'] == 2) //SF
                        $array[]=array('id' =>'start_date_'.$rel['dependent_activity'],'date' =>date('Y-m-d', strtotime($_POST['date'] .' +1 day')));
                }
            } 
         }
        ...
        foreach($items as $items1){
            ...
            foreach($tasks as $item)
            {
                ...
                elseif(empty($workgroupact) && $item['reference_activityid']==0)
                {
                    ...
                    $depRelations = ActivityRelations::find()->Where(['dependent_activity' => $item['id']])->andWhere(['status'=>0])->all();
    ...
    public function actionGetcriticalpath()
    {
        ...
        if(!empty($availableActivites))
        {
            ...
            {          
                ...
                $activity->save(false);
                if($dependentAct->sortorder == $activity->sortorder){
                  $acId = $activity->id;
                  $allRelations = ActivityRelations::find()->Where(['dependent_activity' => $acId])->andWhere(['status'=>0])->all();
                }
                elseif($dependentAct->sortorder > $activity->sortorder){
                  $acId = $activity->id;
                  $allRelations = ActivityRelations::find()->Where(['precedent_activity' => $acId])->andWhere(['status'=>0])->all();
                }
                ...
                if(!empty($allRelations))
                {
                    foreach ($allRelations AS $relation)
                    {
                        ...
                        if($relation->relation_type==1){
                            //SS
                            $lag = $depentAct->lag;
                            $duration = $precAct->duration - 1;
                            $backward_start = date('Y-m-d', strtotime($depentAct->start_date. ' - '.$lag.' days'));
                            $precAct->critical_start = $backward_start;
                            $precAct->critical_end = date('Y-m-d', strtotime($backward_start. ' + '.$duration.' days'));
                        }
                        elseif($relation->relation_type==2){
                            //FS
                            $lag = $depentAct->lag + 1;
                            $duration = $precAct->duration - 1;
                            $backward_end = date('Y-m-d', strtotime($depentAct->start_date. ' - '.$lag.' days'));
                            $precAct->critical_start = date('Y-m-d', strtotime($backward_end. ' - '.$duration.' days'));
                        ...
                        elseif($relation->relation_type==3){
                            ...
                            $lag = $depentAct->lag;
                            $duration = $precAct->duration - 1;
                            if($depentAct->critical_start!='0000-00-00' && $depentAct->critical_end!='0000-00-00'){
                                $backward_end = date('Y-m-d', strtotime($depentAct->critical_end. ' - '.$lag.' days'));
                            }
                            else{
                                $backward_end = date('Y-m-d', strtotime($depentAct->end_date. ' - '.$lag.' days'));
                            }
                            $precAct->critical_start = $precAct->start_date;
                            $precAct->critical_end = $precAct->end_date;
                        }
                        $precAct->save(false);
                    }
                }
            ...
            $newarray = [];
            function GetId($z,$newarray){
                global $newarray;
                $newarray[]= $z;

                $allRelations1 = ActivityRelations::find()->Where(['dependent_activity' => $z])->andWhere(['status'=>0])->all();
                $fdur = '';
                $precActArr = [];
                foreach($allRelations1 as $relation){
                    $precAct1 = Scheduleactivities::findOne($relation->precedent_activity);
                    $fdur1 = $precAct1->duration;
                    if($precAct1->critical_start == $precAct1->start_date){
                        $dependentid = $relation->precedent_activity;
                        
                        if($fdur == '' || $fdur1 >= $fdur){
                            $f = $relation->precedent_activity;
                            if($fdur1 > $fdur)
                                $precActArr = [$relation->precedent_activity];
                            elseif($fdur1 == $fdur)
                                $precActArr[] = $relation->precedent_activity;
                            $fdur = $precAct1->duration;
                        }
                    } 
                }
            ...
            $idarray = GetId($dependentAct->id ,$newarray);

            //---LONGEST PARALLEL ACTIVITIES---------------------
            ...
            if($longestActivites){
                foreach($longestActivites as $longestActivity):
                    $idarray = GetId($longestActivity->id,$newarray);
                endforeach;
            }
            //-----------------------------------------------------
    ...
    public function actionGetitems()
    {
       ...
       // $query = "SELECT * FROM wbsscheduleitems WHERE projectId='" . $_POST['projectid']. "' AND status=0 ORDER BY sortorder ASC";
        ...
        $query="SELECT a.scheduleitem_id,a.wbsid,a.name,a.projectId,b.iowGroupid
                FROM wbsscheduleitems AS a
                INNER JOIN workgroups_new AS b ON  a.wbsid=b.Workgroup_Id
                WHERE  a.projectId='".$_POST['projectid']."' AND a.status=0  ORDER BY b.sortorder  ASC,b.Workgroup_Id ASC";
        $command = $connection->createCommand($query);
        $dataReader = $command->query();
        $items = $dataReader->readAll();
        ...
        foreach($items as $key=>$item){
            // $sql = "SELECT * FROM scheduleactivities WHERE projectId='" . $id . "' AND   scheduleitem_id='".$item['scheduleitem_id']."' ORDER BY projectId ASC ";
            // $command = $connection->createCommand($sql);
            // $dataReader = $command->query();
            // $tasks = $dataReader->readAll();
                
            $iowGroup = IowGroup::find()->where(['id' => $item['iowGroupid'], 'status' => 0])->one();

            $item['item_group'] = '';
            if($iowGroup){
                $item['item_group'] = $iowGroup->name;
            }

            $sql = "SELECT start_date FROM scheduleactivities WHERE projectId='" . $_POST['projectid'] . "' AND   scheduleitem_id='".$item['scheduleitem_id']."' AND status=0 ORDER BY start_date ASC ";
            $command = $connection->createCommand($sql);
            $dataReader = $command->query();
            $strtdate = $dataReader->read();

            $sql = "SELECT end_date FROM scheduleactivities WHERE projectId='" . $_POST['projectid'] . "' AND   scheduleitem_id='".$item['scheduleitem_id']."' AND status=0 ORDER BY end_date DESC ";
            $command = $connection->createCommand($sql);
            $dataReader = $command->query();
            $enddate = $dataReader->read();

            if($strtdate && $enddate){
                $strtdatee=$strtdate['start_date'];
                $enddate=$enddate['end_date'];

                $datediff = strtotime($enddate) - strtotime($strtdatee);
                $totduration = round($datediff / (60 * 60 * 24)) + 1;
                $array[]=$item; 
                $duration[]=$totduration.' Days';
            }
             
        }
        // exit;
        $arr = array('result' => $array,'totduration'=>$duration, 'error' => 'No');
        return json_encode($arr);
    }

    public function actionGetitemactivites()
    {
        // echo $_POST['project']; exit;
        $items = Wbsscheduleitems::findOne($_POST['itemId']);
        $connection = \Yii::$app->db; 
        //$sql = "SELECT * FROM scheduleactivities WHERE scheduleitem_id='".$_POST['itemId']."' AND projectId='".$items->projectId."' AND status=0 ORDER BY sortorder ASC ";
        ...
        $sql="  SELECT s.*,w.act_report_complete_status FROM workgroup_activities_new as w 
                ...
                WHERE w.project_Id = '".$items->projectId."'
        AND w.pricing_status=0 AND s.status=0 AND s.scheduleitem_id='".$_POST['itemId']."' GROUP BY w.id ORDER BY w.sortorder ASC";
        $command = $connection->createCommand($sql);
        $dataReader = $command->query();
        $tasks = $dataReader->readAll();
        ...
        foreach($tasks as $key=>$item){
          ...
          {*/
            ...
            $actualStart        = ($item['act_start_date'] !='' && $item['act_start_date'] != '0000-00-00') ? $item['act_start_date'] : $item['start_date'];
            $endDate            = $item['end_date'];
            $completedQuantity  = 0;

            $datediff       = strtotime($item['actual_end_date']) - strtotime($item['actual_start_date']);
            $duration_first = round($datediff / (60 * 60 * 24)) + 1;
            $dur_first_holiday_cnt   = Yii::$app->helper->getHolidayCount($item['actual_start_date'], $item['actual_end_date'], $item['projectId']);
            $duration_first = $duration_first - $dur_first_holiday_cnt;

            $datediff1      = strtotime($item['end_date']) - strtotime($item['actual_start_date']);
            $fullduration   = round($datediff1 / (60 * 60 * 24)) + 1;
            
            $datediff2      = strtotime($item['act_start_date']) - strtotime($item['actual_start_date']);
            $startDateDiff  = round($datediff2 / (60 * 60 * 24)) + 1;
           

            $datediff3      = strtotime($item['end_date']) - strtotime($item['start_date']);
            $itemDuration   = round($datediff3 / (60 * 60 * 24)) + 1;
            /*$holidayCount   = Yii::$app->helper->getHolidayCount($item['start_date'], $item['end_date'], $item['projectId']);
            $itemDuration   = $itemDuration + $holidayCount;*/

             /*if($item['id'] == 1881){
                echo 'duration_first - '.$duration_first;
                echo "<br>";
                echo 'dur_first_holiday_cnt - '.Yii::$app->helper->getHolidayCount($item['actual_start_date'], $item['actual_end_date'], $item['projectId']);
                echo "<br>";
                echo 'itemDuration - '.$itemDuration;
                echo "<br>";
                echo 'delay - '.$delay = $itemDuration - $duration_first; 
                echo "<br>";
                echo 'delaystart - '.$delaystart = ($delay / $itemDuration) * 100; 
                exit;
            }*/
            ...
            $delaystart = 0;
            if($duration_first < $itemDuration){
                $delay = $itemDuration - $duration_first; 
                if($item['start_date'] > $item['actual_start_date'] && count($relations)==0){
                    $delaystart = ($delay / $fullduration) * 100;
                    //$delaystart = ($delay / $item['duration']) * 100;
                }
            ...
            elseif($startDateDiff > 0){
                $delaystart = ($startDateDiff / $fullduration) * 100;
            }
            
            //If the activity is completed with Performance delay 
            if($delaystart && ($reprtdqty >= $Qty)){
                //$delaystart = 100 - $delaystart;
                $delaystart = 0;
            }
            ...
            //-----Updated on 13-01-2023 - By Sreejith ----

            //Budget Start date will always the start date
            $budgtstart = ((!$item['actual_start_date'] || $item['actual_start_date'] == '0000-00-00' ) ? date('Y-m-d') : $item['actual_start_date']);
            
            //Budget End date will always the end date
            $budgtend = ((!$item['actual_end_date'] || $item['actual_end_date'] == '0000-00-00' ) ? date('Y-m-d') : $item['actual_end_date']);
            
            $datediff = strtotime($budgtend) - strtotime($budgtstart);
            $bduration1 = round($datediff / (60 * 60 * 24)) + 1;
            $bduration = $bduration1 .' Days';

            if($item['old_duration'])
                $bduration = $item['old_duration'] .' Days';
            //-----------------------------------------
            ...
            if($relations){
                ...
                $array[]=array(
                            'id'=> $item['id'], 
                            'name' => '<p>'.$val.$spc.'</p>'.$item['name'], 
                            'activity_name' => $item['name'], 
                            'precedent_activity' => $relation['precedent_activity'], 
                            'dependent_activity' => $relation['dependent_activity'], 
                            'scheduleitem_id' => $item['scheduleitem_id'], 
                            'status' => $item['status'], 
                            'complete_status' => $item['act_report_complete_status'],
                            ...
                            'activity_start'=> $budgtstart, 
                            'activity_end'=> $item['actual_end_date'],
                            'completedQuantity'=>$completedQuantity,
                            'delaystart' => $delaystart, 
                            'relation' => $relationParam,
                            'float'=> $item['float_duration'],
                            'delay'=>$item['delay'],
                            'bduration'=>$bduration,
                            'aduration'=>$aduration,
                            'depstart'=>$scheduleStartdate,
                            'reprtdqty'=>$reprtdqty,
                            'olddur'=>$oldduration,
                            'unit'=>$item['unit']
                        ); 
                ...
                $array[]=array(
                            ...
                            'name' =>'<p>'.$val.$spc.'</p>'.$item['name'], 
                            'activity_name' => $item['name'], 
                            'precedent_activity' => null, 
                            'dependent_activity' => null, 
                            'scheduleitem_id' => $item['scheduleitem_id'], 
                            'status' => $item['status'], 
                            'complete_status' => $item['act_report_complete_status'],
                            'projectId' => $item['projectId'], 
    ...
    public function actionNewganttchartshow($id)
    {
        Yii::$app->helper->GetRelationcorrect($id);;
        $connection = \Yii::$app->db; 
        $query = "SELECT * FROM wbsscheduleitems WHERE scheduleitem_id='" . $id . "' AND status=0";
        $command = $connection->createCommand($query);
        $dataReader = $command->query();
        $items = $dataReader->readAll();
        $html = '';
        $relationsHtml = '';
        foreach($items as $item){
            $sql = "SELECT * FROM scheduleactivities WHERE scheduleitem_id='".$item['scheduleitem_id']."' AND status=0 ORDER BY sortorder ASC ";
            $command = $connection->createCommand($sql);
            $dataReader = $command->query();
            $tasks = $dataReader->readAll();
            // var_dump($tasks);
        ...
        /*$pdf = Yii::$app->pdf;
        $pdf->content = $html;  
        return $pdf->render('ganttprintview', array(
          'activities' => $html,
          'scheduleitemId' => $id,
          'relationsHtml' => $relationsHtml,
        ));*/
        ...
        return $this->render('ganttprintview', array(
          'activities' => $html,
          'scheduleitemId' => $id,
          'relationsHtml' => $relationsHtml,
        ));
    ...
    public function actionGetitemactivitess()
    {
        // echo $_POST['project']; exit;
        $items = Wbsscheduleitems::findOne($_POST['itemId']);
        $connection = \Yii::$app->db; 
        //print_r($items);exit;
        $sql = "SELECT * FROM scheduleactivities WHERE scheduleitem_id='".$_POST['itemId']."' AND status=0 ORDER BY sortorder ASC ";
        //echo $sql;exit;
        $command = $connection->createCommand($sql);
        $dataReader = $command->query();
        $tasks = $dataReader->readAll();
     ...
     public function actionAllganttchart($id)
    {
        Yii::$app->helper->GetRelationcorrect($id);;
        $connection = \Yii::$app->db; 
        $query = "SELECT * FROM wbsscheduleitems WHERE projectId='" . $id . "' AND status=0";
        ...
        //$this->layout = 'gantlayout';
        //$this->layout = false;
        return $this->render('ganttprintallview', array(
          'activities' => $html,
          'projectId' => $id,
          'relationsHtml' => $relationsHtml,
        ));
