...
<!-- -->
 ...
 <div id="A">
    <form id="activities_edit_form">
      ...
      <table class="table schedule-edit-table" style="text-align: left;">
          <thead>
            <tr>
              <th>Activities</th>
    ...
    <!--  -->
<div id="B">
    <a class="btn ganttback ganttbacksch" style="display: none;position: relative;width:100px;float: right;
    margin-bottom: 3px;
    background: #072c47;
    border-radius: 20px;
    color: #fff;margin-right:20px;" href="<?php echo Yii::$app->request->baseUrl;?>/projectsmain/index#back">Back</a>

    <a class="btn ganttback ganttbackschlist" style="display: none;position: relative;width:100px;float: right;
    margin-bottom: 3px;
    background: #072c47;
    border-radius: 20px;
    color: #fff;margin-right:20px;" href="<?php echo Yii::$app->request->baseUrl;?>/projectsmain/index#backact">Back</a>
   ...
   <!-- <button class="btn edit_relation" style="position: relative;width:100px;float: right;
    ...
    <div id="embedded-Gantt">
      <div style="padding: 50px; text-align: center; background-color: #ffffff;">Loading....</div>
    </div>
    ...
    <div style="padding-top: 20px;">
      <table class="legendTable">
        <tr>
            <td>
              <div><span class="legendColorBox legendStartDelay"></span> Start Delay</div>
            </td>
            <td>
              <div><span class="legendColorBox legendOngoing"></span> Ongoing</div>
            </td>
            <td>
              <div><span class="legendColorBox legendCritical"></span> Critical</div>
            </td>
            <td>
              <div><span class="legendColorBox legendPerformanceDelay"></span> Peformance Delay</div>
            </td>
            <td>
              <div><span class="legendColorBox legendCompleted"></span> Completed</div>
            </td>
...
function loadGantt()
{
  ...
  if (g.getDivId() != null) {
        ...
        success: function(data1){
          // var items=JSON.parse(data.result);
            ...
            for (j = 0; j < data1.result.length; j++) {
              ...
              //finalActualEndDate = addOneDay(new Date(finalActualEndDate));
    ...
    g.AddTaskItem(new JSGantt.TaskItem(0,   'Project - <?= $projectName ?>', '', '', 'ggroupblack',  '', 0, '<?= $projectDuration ?>', 100, 1, 0, 1, '', '', 'Some Notes text', g));
    ...
    $.ajax({
          ...
          success: function(data){
              ...
              for (i = 0; i < data.result.length; i++) {
                ...
                var dur = data.totduration[i];

                if(item.item_group && !iow_group_arr.includes(item.item_group)){
                  g.AddTaskItem(new JSGantt.TaskItem(item.item_group,   '<span  class="item_group">'+item.item_group+'</span>', '', '', 'iow-group',  '', 0, '', 100, 1, 0, 1, '', '', 'Some Notes text', g));
                  iow_group_arr.push(item.item_group);
                }
                if(item.item_group){
                  //indent_space = '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;';
                  item_group_indent_class = 'item_group_indent';
                }
                else{
                  //indent_space = '';
                  item_group_indent_class = '';
                }

                g.AddTaskItem(new JSGantt.TaskItem(item.scheduleitem_id,   '<span class="'+item_group_indent_class+'"  title="'+item.name+'">'+item.name+'</span>', '', '', 'ggroupblack',  '', 0, dur, 100, 1, 0, 1, '', '', 'Some Notes text', g ));
                ...
                $.ajax({
                    ...
                    success: function(data1){
                        for (j = 0; j < data1.result.length; j++) {
                            ...
                            var totalQuantity = activity.quantity;
                            //var completedQuantity = activity.completedQuantity;
                            var completedQuantity = activity.reprtdqty;
                            if(totalQuantity == completedQuantity || completedQuantity == null || completedQuantity == 0 )
                            {
                              // var percentageDifference = (actualDayDifference/budgetedDayDifference)*100;
                              if(completedQuantity ==null)
                            ...
                            //finalActualEndDate = addOneDay(new Date(finalActualEndDate));
                            finalActualEndDate = activity.actual_end_date;
                            ...
                            if(activity.activity_end < activity.actual_end_date && Number(activity.reprtdqty) 
                              && activity.actual_start_date != activity.actual_end_date 
                              && activity.activity_start != activity.activity_end
                              && Number(activity.reprtdqty) < Number(activity.quantity)

                              ){
                              //Performance delay for reported activities
                              //var planenddate = activity.activity_end;

                              planenddate_temp = new Date(activity.actual_start_date);
                              planenddate_temp.setDate(planenddate_temp.getDate()+(parseInt(activity.bduration)-1));
                              var planenddate = planenddate_temp;
                            ...
                            var bdate = new Date(activity.activity_start);
                            ...
                            var bdateorg = bday + "/" + bmonth + "/" + byear;

                            var bdateend = new Date(activity.activity_end);
                            var bdayend = bdateend.getDate();
                            var bmonthend = bdateend.getMonth() + 1;
                            var byearend = bdateend.getFullYear();
                            if (bdayend < 10) {
                                bdayend = "0" + bdayend;
                            }
                            if (bmonthend < 10) {
                                bmonthend = "0" + bmonthend;
                            }
                            var bdateendorg = bdayend + "/" + bmonthend + "/" + byearend;
                            var sp = ' ';
                            var qty = activity.quantity + sp + activity.unit;
                            ...
                            /*if(activity.float>0){
                              ...
                              //Commented by Sreejith and enabled above one
                              //var finalpercentagecompleted = activity.delaystart;

                            //}

                            p_end = addOneDay(new Date(actualenddate));
                            planenddate = addOneDay(new Date(planenddate));
                            //planstartdate = addOneDay(new Date(planstartdate));
                            //actualstartdate = addOneDay(new Date(actualstartdate));

                            if(Number(activity.delaystart) > 0 && Number(activity.reprtdqty) >= Number(activity.quantity)){
                              var finalpercentagecompleted = (100 - activity.delaystart);

                              //If the activity is completed without Performance delay
                              if(activity.reprtdqty >= activity.quantity && activity.activity_end == activity.actual_end_date){
                                var finalpercentagecompleted = 0;
                              }
                            }
                            else if(Number(activity.reprtdqty) > 0){
                              //var finalpercentagecompleted = (100 - percentagecompleted);
                              var finalpercentagecompleted = activity.delaystart;
                            }

                            if(Number(activity.reprtdqty) >= Number(activity.quantity)){
                               // var finalpercentagecompleted = 0; 
                            }
                            ...
                            if($.inArray(activity.id, criticalid) !== -1){
                                ...
                                if(activity.activity_start < activity.actual_start_date){
                                  ...
                                  $class = 'gtaskblue';
                                  if(activity.activity_start < activity.actual_start_date){
                                    //Start Delay => Light gray color
                                    $class = 'gtaskblue';
                                  }
                                  else if(actualenddate > activity.activity_end && Number(activity.reprtdqty) > 0){
                                    //Performance Delay => Red color 
                                    $class = 'gtaskgreen';
                                  }
                            }


                            if((Number(activity.quantity) <= Number(activity.reprtdqty)) ){
                              //Completed Activity => Green Color
                              $class = 'gtaskcompleted gtaskyellow';
                              //if(activity.activity_start < activity.actual_start_date || actualenddate > activity.activity_end){
                              if(activity.activity_start < activity.actual_start_date){
                                $class = 'gtaskyellow';
                              }
                            }
                            else if(activity.activity_start >= activity.actual_start_date && activity.actual_end_date <= activity.activity_end){
                              finalpercentagecompleted = 0;
                              $class = 'gtaskblue '+$class;
                              if($.inArray(activity.id, criticalid) == -1){
                                $class = 'gtaskongoing '+$class;
                              }
                            }
                            
                            if(parseInt(activity.complete_status)){
                              //Completed Activity => Green Color
                              $class = 'gtaskcompleted gtaskyellow';
                              finalpercentagecompleted = 0;
                            }
                            ...
                            if($test = 0){ //--TEST--
                              currentIteration_i = parseInt(i)+1;
                              currentIteration_j = parseInt(j)+1;

                              if(currentIteration_i == 3 && currentIteration_j == 3 ){
                                
                                //actualstartdate = "2023-01-01"; // pPlanStart
                                planenddate = "2023-01-1"; // pPlanEnd
                                //planstartdate = "2023-01-02"; // pStart
                                //p_end = "2023-01-04"; // pEnd
                              }
                              if(currentIteration_i == 2 && currentIteration_j == 1 ){
                                
                                //actualstartdate = "2023-01-01"; // pPlanStart
                                planenddate = "2023-01-16"; // pPlanEnd
                                //planstartdate = "2023-01-02"; // pStart
                                //p_end = "2023-01-04"; // pEnd
                              }
                          ...
                          taskItemObject = {
                              pID: activity.id+random,
                              pName: '<span class="'+item_group_indent_class+'" title="'+activity.activity_name+'">'+activity.name+'</span>',
                              
                              //---Removed start delay----
                              //pStart: planstartdate,
                              //pEnd: actualenddate,
                              //pStart: '',
                              //pEnd: '',
                              //---------------------
                              pStart: planstartdate,
                              pEnd: p_end,
                              //pEnd: addOneDay(new Date(actualenddate)),

                              pPlanStart: actualstartdate,
                              pPlanEnd: planenddate,
                              //pPlanEnd: addOneDay(new Date(planenddate)),
                              pClass : $class,
                              pLink : "",
                              pMile : 0,
                              pRes : activity.aduration,
                              pComp: finalpercentagecompleted,
                              pGroup: 0,
                              pParent: activity.scheduleitem_id,
                              pOpen: 1,
                              pDepend: relation1,
                              pCaption: "",
                              pCost: qty,
                            ...
                            g.AddTaskItemObject(taskItemObject);
...
$(document).on('click','.gselector .gformlabel',function(){
  changeEndDates();
});
...
/*$(document).ready(function () {  
  ...
  /*$(document).on( "click", ".ganttback", function(){ 
    //alert("hi")
    //$('.projects-tab ').addClass('active');
    window.location = '../projectsmain/index#';
...
$( "#ganttchartitems" ).sortable({
    ...
    function changeEndDates() {
      $( ".genddate" ).each(function( index ) {
        endDateDiv = $( this ).find( "div" );
        if(endDateDiv.text()){
          endDateDiv.html(reduceOneDay(endDateDiv.text(), 1));
        }
      });
    }
    ...
    function reduceOneDay(date, splitNeeded = 0) {
      if(splitNeeded){
