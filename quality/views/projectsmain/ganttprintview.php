...
<div id="B">
   ...
   <!-- <button class="btn edit_relation" style="position: relative;width:100px;float: right;
    ...
    background: #072c47;
    border-radius: 20px;
    color: #fff;margin-right:20px;display:none; ">Relations</button> -->
    <div id="embedded-Gantts"></div>
  </div>
<!-- -->
...
function loadGantt()
{
  var g = new JSGantt.GanttChart(document.getElementById('embedded-Gantts'), 'month');
  if (g.getDivId() != null) {
    ...
    g.setDayMajorDateDisplayFormat('mon yyyy - month ww') // Set format to display dates in the "Major" header of the "Day" view
    ...
    $.ajax({
        type: 'POST',
        url: '../projectsmain/getstructuresactivites',
        dataType: "json",
        async:false,
          ...
          success: function(data){
            // var items=JSON.parse(data.result);
              ...
              for (i = 0; i < data.result.length; i++) {
                ...
                $.ajax({
                    type: 'POST',
                    url: '../projectsmain/getitemactivitess',
                    dataType: "json",
                    async:false,
                    data: {scheduleitemid:<?= $scheduleitemId ?>, itemId: item.scheduleitem_id},
                    success: function(data1){
                      // var items=JSON.parse(data.result);
                        // console.log(data1.result);
                        // alert(data.error);
  ...
  loadGantt();
   $('.save-relation').click(function(){
    ...
    $.ajax({
            ...
            dataType: "json",
            data: {id:relation,firstItem: firstItem, firstActivity: firstActivity, secondItem: secondItem, secondActivity: secondActivity, relationType: relationType,lag:lag},
            success: function(data){
                if(data.error=='No')
                {
                    loadGantt();
