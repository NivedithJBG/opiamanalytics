...
<form method="POST" action="<?php echo Yii::$app->request->baseUrl.'/procurement/purchaseorder' ?>" id="placeorderform" autocomplete="off">  
	<input type="hidden" name="activityData" value='<?php echo json_encode($activityData) ?>'>		 
	<div class="purchase-order2-wrpr" style="padding:0px; padding-bottom:0px;padding-top: 0;">
		    	         	...
		    	         	<div class="col-md-12">    
			                  ...
			                  <?php 
			                  	if($orderDataArr){
			                  		...
			                  		foreach ($orderDataArr as $key => $orderData) {
			                  			...
			                  			$amount = ($orderData['rate'] * $orderData['quantity']);
			                  			$total_amount += $amount;
			                  ...
			                  ?>      		
	                   		<tr>
	                            ...
	                            <td>
	                            	<input type="text" class="form-control size cgsttt cgstvalue<?php echo $activity ?>" name="gst[]" id="cgst" data-id="<?php echo $activity ?>">
	                            </td>
	                            <td>
	                            	<input type="text" class="form-control size igsttt igstvalue<?php echo $activity ?>" name="igst[]" id="igst" data-id="<?php echo $activity ?>">
	                            	<input type="hidden" class="form-control itemTaxValue" id="taxvalue<?php echo $activity ?>">
	                            	<input type="hidden" class="amntvalue<?php echo $activity ?>" value="<?php echo number_format((float)$amount, 2, '.', '')  ?>">
	                            </td>
							...
							<div class="col-md-3">
								<div class="row">
									<div class="">
										...
										<input type="text" class="form-control" name="order_number" id="order_number" value="<?php echo $order_number ?>">
									</div>                                    
								</div>
							</div>
							...
							<div class="col-md-3">
								<div class="row">
									<div>
										...
										<input type="date" class="form-control" name="orderofdate"  value="<?php echo date('Y-m-d') ?>">
									</div>
								</div>
							</div>
			                          ...
			                          <div class="col-md-3">
			                                <div class="row">
			                                    <div>
			                                        ...
			                                        <input type="date" class="form-control" name="dateofdelivery" value="<?php echo date('Y-m-d', strtotime(date('d-m-Y'). ' + '.$resource->lead_time.' days')) ?>">
			                                        <span class="error"></span>
			                                    </div>
		                     		...
		                     		<div class="col-md-3" style="height:76px">
		                     			<div class="row">
		                     				<div>
		                     					...
		                     					<!-- <input type="text" name="" id="credperiod" name="creditperiod" value="<?php echo $resource->credit_period ?>"> -->
		                     					<select id="credperiod" name="creditperiod">
		                     						<option value="none">Select Credit Period</option>
		                     						<option value="0 Days">Instantly</option>
		                     						<option value="15 Days" <?php echo ($resource->credit_period == 15) ? 'selected' : '' ?> >15 Days</option>
		                     						<option value="30 Days" <?php echo ($resource->credit_period == 30) ? 'selected' : '' ?>>30 Days</option>
		                     						<option value="45 Days" <?php echo ($resource->credit_period == 45) ? 'selected' : '' ?>>45 Days</option>
		                     						<option value="60 Days" <?php echo ($resource->credit_period == 60) ? 'selected' : '' ?>>60 Days</option>
		                     						<option value="90 Days" <?php echo ($resource->credit_period == 90) ? 'selected' : '' ?>>90 Days</option>
			                            ...
			                            <div class="col-md-12 text-center india" style="margin-top:20px;display:none">
		                             	...
		                             	<div class="col-md-12 text-center " style="margin-top:30px;">
		                                	<button type="button" value="Cancel" class="btn btn-primary cancel" id="cancelorder" data-dismiss="modal"><span class="icon-cancel5"></span> &nbsp; Cancel</button>
		                                	<button type="submit" value="submit" id="purchaseorderbtn" class="btn btn-primary" name="submit"><span class="icon-email1"></span> &nbsp; Mail to Vendor</button>
		                            	</div>
		                        	</div>
		                    	</div>
	                    	</div>
