...
$(function(){
    ...
    $('#saveproject').click(function(){
        ...
        if(error==0){
            ...
            addFormData.append('projectname', $('#projectnameover').val());
            addFormData.append('projectvalue', projectValue);
            addFormData.append('clientname', $('#clientname').val());
            addFormData.append('location', $('#projectlocationover').val());
            addFormData.append('duration', $('#duration').val());
            addFormData.append('enddate', $('#enddate').val());
            addFormData.append('startdate', $('#startdate').val());
            addFormData.append('wrkhrss', $('#wrkhrs').val());
            addFormData.append('cashaccount', $('#cashaccount').val());
            addFormData.append('bankaccount', $('#bankaccount').val());
            if ($('#project_documents').length && $('#project_documents')[0].files) {
                $.each($('#project_documents')[0].files, function(i, file) {
                    addFormData.append('project_documents[]', file);
                });
            }
            if ($('#project_correspondence').length && $('#project_correspondence')[0].files) {
                $.each($('#project_correspondence')[0].files, function(i, file) {
                    addFormData.append('project_correspondence[]', file);
                });
            }
            $.ajax({
                type:'POST',
...
$(document).on('click','.editprojectbutton',function(){
        ...
        $.ajax({
            ...
            success: function(data){
                 // alert(data.durat);
                    ...
                    $('#editprojectlocation').val(data.location || '');
                    ...
                    var docsHtml = '', corrHtml = '';
                    if (data.files && data.files.length) {
                        $.each(data.files, function(i, f) {
                            var link = '<a href="../web/uploads/projects/' + f.filename + '" target="_blank">' + f.original_name + '</a>';
                            var del = ' <a href="#" class="delete-project-file text-danger" data-id="' + f.id + '">[x]</a><br>';
                            if (f.file_type === 'documents') docsHtml += link + del;
                            else corrHtml += link + del;
                        });
                    }
                    $('#existing_documents').html(docsHtml || '<em class="text-muted">No files uploaded</em><br>');
                    $('#existing_correspondence').html(corrHtml || '<em class="text-muted">No files uploaded</em><br>');
    ...
    $('.error').hide();
        ...
        if(error==0){
            ...
            editFormData.append('projectid', idval);
            editFormData.append('projectname', $('#projectnames').val());
            editFormData.append('startdate', $('#startdatee').val());
            editFormData.append('enddates', $('#enddatee').val());
            editFormData.append('duration', $('#durationn').val());
            editFormData.append('projectvalue', $('#projectvalues').val());
            editFormData.append('clientname', $('#clientnames').val());
            editFormData.append('location', $('#editprojectlocation').val());
            editFormData.append('wrkhrs', $('#wrkhrss').val());
            editFormData.append('cashaccount', $('#editcashaccount').val());
            editFormData.append('bankaccount', $('#editbankaccount').val());
            $.each($('#edit_project_documents')[0].files, function(i, file) {
                editFormData.append('project_documents[]', file);
            });
            $.each($('#edit_project_correspondence')[0].files, function(i, file) {
                editFormData.append('project_correspondence[]', file);
            });
        $.ajax({
            type: 'POST',
            url: '../projects/edit',
...
$(document).on('click', '.delete-project-file', function(e) {
    e.preventDefault();
...
$(document).on('change', '.custom-file-input', function() {
    ...
    $(this).next('.custom-file-label').text(names || 'Choose files');
});

// Project Documents — save button below each card
$(document).on('click', '.project-doc-save', function(e) {
    e.stopPropagation();
    var $btn      = $(this);
    var $wrap     = $btn.closest('.project-doc-upload');
    var projectId = $btn.data('id');
    var input     = $wrap.find('.project-doc-input')[0];
    if (!input.files.length) { alert('Please select files first.'); return; }
    var fd = new FormData();
    fd.append('project_id', projectId);
    $.each(input.files, function(i, file) { fd.append('pf_documents[]', file); });
    var origHtml = $btn.html();
    $btn.attr('disabled', true).html('<span class="icon-spinner3"></span>');
    $.ajax({
        type: 'POST', url: '../projectsmain/uploadprojectfiles',
        data: fd, contentType: false, processData: false, dataType: 'json',
        success: function(data) {
            $btn.attr('disabled', false).html(origHtml);
            if (data.error === 'No') {
                input.value = '';
                var $flash = $('<div>File Saved</div>').css({
                    position: 'fixed', top: '80px', left: '50%', transform: 'translateX(-50%)',
                    background: '#28a745', color: '#fff', padding: '10px 28px',
                    borderRadius: '4px', fontSize: '15px', fontWeight: '600',
                    zIndex: 9999
                });
                $('body').append($flash);
                setTimeout(function(){ $flash.fadeOut(400, function(){ $flash.remove(); }); }, 3000);
            } else { alert('Upload failed: ' + (data.errortext || 'unknown error')); }
        },
        error: function(xhr) {
            $btn.attr('disabled', false).html(origHtml);
