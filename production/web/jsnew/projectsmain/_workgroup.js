    ...
    $(document).on('click','.saveworkgroupbutton',function(){
        ...
        if(error==0)
        {
            $.ajax({
                ...
                dataType: "json",
                data: {workid:idval,name:$('#editworkgroupname'+idval).val(),worktype:$('#editworktype'+idval).val(),iowgroupid: $('#editiowgroupid'+idval).val()},
                success: function(data){
                    if(data.error=='No')
                    {
       ...
       /* if($('#workgroupname').val()!='' && WorkGroupNameExists($('#workgroupname').val(),$('#selectedProjectId').val())=='Yes')
        ...
        if(error==0)
        {
            $.ajax({
                ...
                dataType: "json",
                data: {Project_Id:$('#selectedProjectId').val(),workgroupname:$('#workgroupname').val(),iowgroupid:$('#iowgroupid').val(),worktype:$('#worktype').val(),worktypegroup:$('#worktypegroups').val()},
                success: function(data){
...
function initWbsSortable() {
    if (!$('#wbs-list-tbody').length || !$.fn.sortable) return;
    $('#wbs-list-tbody').sortable({
        placeholder: 'ui-state-highlight',
        helper: 'clone',
        update: function(event, ui) {
            var updatedrows = [];
            $('.listdats').each(function() {
                updatedrows.push({ rowid: $(this).attr('data-id'), rowindex: $(this).index() });
            });
            $.ajax({
                type: 'POST',
