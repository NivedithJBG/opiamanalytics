...
function renderCdResourceConsumption(items, actName, lastQty){
    ...
    function fmR(v){ return v >= 1000000 ? (v/1000000).toFixed(1)+'M' : v >= 1000 ? (v/1000).toFixed(1)+'K' : (+v).toFixed(2) == Math.round(+v) ? (+v).toFixed(0) : (+v).toFixed(2); }

    // Total quantity across all resources
    var totalQty = 0;
    items.forEach(function(r){ totalQty += (+r.res_qty || 0); });

    if (!items.length || !totalQty){
        el.innerHTML = '<div style="text-align:center;font-size:12px;color:#5a6e8c;padding:18px 0">No estimate data</div>';
        return;
    }

    // Group by resource type — sum qty per type, collect individual resources
    var typeMap = {};
    items.forEach(function(r){
        var tid = r.type_id || '0';
        if (!typeMap[tid]) typeMap[tid] = { name: r.type_name || 'Other', amount: 0, resources: [] };
        typeMap[tid].amount += (+r.res_qty || 0);
    ...
    // Bars (flex-grow spacer trick) — data-cons-idx for tooltip binding
    var barsHtml = '';
    var labelsHtml = '';
    types.forEach(function(t, i){
        var pct = t.amount / totalQty * 100;
        var col = colPalette[i % colPalette.length];
        var sp  = Math.max(100 - pct, 0).toFixed(2);
        var bp  = Math.max(pct, 0.5).toFixed(2);
    ...
    // Shared tooltip element for consumption panel
    ...
    el.querySelectorAll('[data-cons-idx]').forEach(function(col){
        col.addEventListener('mouseenter', function(){
            ...
            // Y-axis scale labels — actual qty values
            var tsHtml = '';
            [100,75,50,25,0].forEach(function(g){
                tsHtml += '<div style="position:absolute;right:2px;bottom:calc(' + g + '% - 5px);'
                    + 'font-family:\'Nunito\',sans-serif;font-size:9px;color:#667;line-height:1;white-space:nowrap;">'
