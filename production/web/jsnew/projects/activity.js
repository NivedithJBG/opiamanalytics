...
$(function(){
    ...
    function doActivitySearch() {
        $.ajax({
            ...
            dataType: "json",
            data: {Workgroup_Id:$('#selectedWorkgrouId').val(),activitytypeid:$('#wbsactivitytypelist').val(),worktypeid:$('#wbsworktypelist').val(),activityname:$('#searchenggactname').val()},
            success: function(data){
                if(data.error=='No')
                {
                    $('#iowtables').show();
    ...
    $('#enggactsearch').click(doActivitySearch);

    var activitySearchTimer;
    $(document).on('input', '#searchenggactname', function(){
        clearTimeout(activitySearchTimer);
        activitySearchTimer = setTimeout(doActivitySearch, 300);
    });
    // add iow click
    // save iow click
...
/*$(document).on('change','#worktypelistactivities',function(){
    
    var worktypeid = $('#worktypelistactivities').val();
    ...
    $.ajax({
            ...
            dataType: "json",
            data: {Workgroup_Id:$('#selectedWorkgrouId').val(),worktypeid:worktypeid},
            success: function(data){
                if(data.error=='No')
                {
                    $('#deletedactivityitems').html(data.result);
...
$(document).on('click','.saveiowactivitybutton',function(){
    $.ajax({
        type: 'POST',
        url: '../activity1/saveiowactivities',
        beforeSend : function(){
            $('.saveproactivitybutton').attr("disabled", true);
        },
        dataType: "json",
...
$(document).on('click','.addiowactivitydrop',function(){
    ...
    //     var schedule=$('#schedule'+actid).val();
    //     //var schedule= 1;
    // }
    // else {
    ...
    //         var estimate = 0;
    //     }
    //     if ($('#checkschedule'+actid).is(':checked')){
    //         var schedule = 1;
    //     }
    //     else {
    //         var schedule = 0;
