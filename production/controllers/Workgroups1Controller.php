...
class Workgroups1Controller extends \yii\web\Controller
{
    ...
    public function actionSearch()
    {
        ...
        if($projuser)
        {
		    ...
		    $sql="SELECT Workgroup_Id,Project_Id,Name,worktype,iowGroupid FROM workgroups_new WHERE Status='0' ";
			...
			if(count($dataProvider)>0):
                $datarows .= '<style>
                    #wbs-list-wrap{margin:0 -8px;}
                    #wbs-list-table{width:100%;border-collapse:collapse;margin-bottom:0;font-size:14px;table-layout:auto;}
                    #wbs-list-table thead th{background-color:#555555;color:#fff;font-weight:bold;padding:10px 12px;text-align:center;border:1px solid #444;white-space:nowrap;}
                    #wbs-list-table thead th:nth-child(2){text-align:left;}
                    #wbs-list-table tbody td{padding:8px 10px;border:1px solid #aaa;color:#222;vertical-align:middle;text-align:center;white-space:nowrap;}
                    #wbs-list-table tbody td:nth-child(2){text-align:left;white-space:normal;min-width:180px;}
                    #wbs-list-table tbody tr:hover{background-color:#f9f9f9;}
                    #wbs-list-table .form-control{padding:4px 7px;height:30px;font-size:14px;}
                    #wbs-list-table tbody td:last-child{padding:5px 8px;white-space:nowrap;}
                    #wbs-list-table .listactivity{padding:4px 16px;font-size:13px;border-radius:50px;line-height:1.5;}
                    #wbs-list-table .editworkgroupbutton,#wbs-list-table .saveworkgroupbutton,#wbs-list-table .deleteworkgroupbutton{width:32px;height:32px;border-radius:50%;padding:0;line-height:30px;text-align:center;margin-left:6px;font-size:14px;}
                </style>
                <div id="wbs-list-wrap"><table id="wbs-list-table" class="table table-bordered">
                <thead><tr>
                    <th>#</th>
                    <th>IOW Name</th>
                    <th>Group Name</th>
				...
				foreach($dataProvider AS $key=>$data):
                    ...
                    $iowGroups = IowGroup::find()->where(['status' => 0])->orderBy('name ASC')->all();
                    ...
                    $datarows .= '<tr class="listdats datass" id="workgrouprow'.$data['Workgroup_Id'].'" data-id="'.$data['Workgroup_Id'].'">
                        <td>'.($key + 1).'<input type="hidden" value="'.$data['Workgroup_Id'].'"></td>
                        <td>
                            <span class="prjctlist" id="workgroupname'.$data['Workgroup_Id'].'">'.$data['Name'].'</span>
                            <input class="form-control editworkgroupname" style="display:none;margin-top:2px;" type="text" id="editworkgroupname'.$data['Workgroup_Id'].'" value="'.$data['Name'].'">
                            <span class="error"></span>
                        </td>
                        <td>
                            <span id="iowGroupId'.$data['Workgroup_Id'].'">'.$iowGroupName.'</span>
                            <select class="form-control" id="editiowgroupid'.$data['Workgroup_Id'].'" name="iowgroupid" style="display:none;margin-top:2px;">
                                '.$iowGroupSelectBox.'
                            </select>
				...
				$datarows = '<style>
                    #wbs-list-wrap{margin:0 -8px;}
                    #wbs-list-table{width:100%;border-collapse:collapse;margin-bottom:0;font-size:13px;table-layout:auto;}
                    #wbs-list-table thead th{background-color:#555555;color:#fff;font-weight:bold;padding:10px 12px;text-align:center;border:1px solid #444;white-space:nowrap;}
                    #wbs-list-table thead th:nth-child(2){text-align:left;}
                </style>
                <div id="wbs-list-wrap"><table id="wbs-list-table" class="table table-bordered">
                <thead><tr><th>#</th><th>IOW Name</th><th>Group Name</th><th></th></tr></thead>
                <tbody><tr><td colspan="4" class="text-center" style="padding:20px;">No Workgroup Found</td></tr></tbody>
                </table></div>';
			endif;
    ...
    {
        $iowGroup = IowGroup::find()->where(['name' => $_POST['iowgroupname']])->one();
        if($iowGroup){
            $arr = array('error'=>'Yes','errortext'=>'IOW group already exists!');
            return json_encode($arr);
        }
