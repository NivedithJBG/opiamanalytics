...
<script src="<?php echo Yii::app()->request->baseUrl; ?>/codebase/dhtmlxgantt_critical_path.js"></script>
...
<script type="text/javascript">
    ...
    $(function(){
        unavailableDates =[ <?php echo $calander;?> ];
...
<title>Gant view</title>
    <div style="display: none;" id="datesdiv" ><?php echo '['.$calander.']';?></div>
...
<div style="width:100%;height: 30px;margin-top: 10px;">
    ...
    <div style="padding:5px;background-color:#0066cc;color: white;font-family: arial;font-size: 13px;float: left;margin: 5px;">IOW Actual</div>
    <div style="padding:5px;background-color:#8EDFF0;color: white;font-family: arial;font-size: 13px;float: left;margin: 5px;">IOW Budgeted</div>
    ...
    <a href="<?php echo Yii::app()->request->baseUrl; ?>/projects/deletelinks/<?php echo $model->Project_Id;?> " onclick="return confirm('Are you sure want to delete all links ?');" ><button type="button" id="deletelinks" style="padding:5px;border:0px;background-color:rgba(27,200,242,0.77);color: white;font-family: arial;font-size: 13px;float: left;margin: 5px;">Delete links</button></a>
...
<script type="text/javascript">
    ...
    gantt.config.columns = [
        {name:"text",       label:"Task name",  width:"200",align: "left", tree:true },
    ...
    gantt.attachEvent("onBeforeLinkAdd", function(id,item){
        ...
        if(source.itemtype=='iow' && target.itemtype=='iow'){
            var itemsource = gantt.getTask(item.source);
            var itemtarget=gantt.getTask(item.target);
            if(itemtarget.id>itemsource.id)
            {
                return true;
            }
            else
            {
                alert('Can not add link to an item which is greater in order')
                return false;
            }
        }
        else
        {
            var source =''
                if(source.itemtype=='project' || target.itemtype=='project')
                {
                    item ='Project';
                }
                else
            ...
            alert("You can not create links with "+item)
            return false;
        }
        return false;
    });
...
/*        var task = gantt.getTask(id);
    ...
    var dp=new dataProcessor("<?php echo Yii::app()->request->baseUrl; ?>/projects/Updatechart");
