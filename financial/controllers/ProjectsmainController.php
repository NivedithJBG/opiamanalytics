...
class ProjectsmainController extends Controller
{
	...
	public function actionSearchscheduleitem()
    {
        ...
        if($projuser)
        {
			...
			$gantt = '<a class="btn btn-primary text-button listactssganttback" value="' . $projuser->projectid . '" href="' . Yii::$app->request->baseUrl . '/projectsmain/newganttchart?id=' . $projuser->projectid . '#schgantt" title="Click to view Gantt chart"><span class="icon-chart6" style="font-size: 15px;"></span>  Gantt Chart</a>';
                      
            $arr = array('result' => $datarows, 'gantt'=> $gantt, 'error'=>'No');
            return json_encode($arr);
	...
	public function actionListscheduleactivities()
    {
        ...
        if($projuser){
            ...
            $connection = Yii::$app->db;
            //$sql="SELECT * from scheduleactivities WHERE status=0 AND scheduleitem_id='".$_POST['itemId']."' AND projectId='".$projuser->projectid."' ORDER BY sortorder ASC ";
            //echo $sql;
            $sql="SELECT a.reference_activityid,a.id,a.name,a.actual_start_date,a.actual_end_date,a.start_date,a.end_date,a.activity_id,a.process,a.unit,a.old_duration,a.projectId FROM scheduleactivities AS a
                INNER JOIN workgroup_activities_new AS b ON  a.activity_id=b.id
                WHERE a.scheduleitem_id='".$_POST['itemId']."' AND a.projectId='".$projuser->projectid."' AND a.status=0  ORDER BY b.sortorder ASC";
            ...
            $datarows='<div class="col-md-12">
                            <div class="row sacthds">
                                ...
                                <div class="col-md-2"  style="padding-left: 0px;margin-left: -12px;">
            ...
            if(count($dataProvider)>0):
                ...
                //if(count($Relations1)==0 && count($Relations2)==0 && $data['start_date']!='' && $data['end_date']!='' && $data['start_date']!='0000-00-00' && $data['end_date']!='0000-00-00'){ change made in 21/12/2021
                   ...
                   // $start_date = date("d-m-Y", strtotime($data['start_date']) );change made in 21/12/2021
                   // $end_date = date("d-m-Y", strtotime($data['end_date']) );change made in 21/12/2021
              ...
              //if($data['start_date']!='' && $data['end_date']!='' && $data['start_date']!='0000-00-00' && $data['end_date']!='0000-00-00'){change made in 21/12/2021
                if($data['actual_start_date']!='' && $data['actual_end_date']!='' && $data['actual_start_date']!='0000-00-00' && $data['actual_end_date']!='0000-00-00'){
                        ...
                        $end_date = date("d-m-Y");
                        $start_date_edit = date("Y-m-d");
                        $end_date_edit = date("Y-m-d");
                        $start_date_visible = '';
                        $end_date_visible = '';
                        $duration_visible = '';
                    }
                  ...
                  elseif(empty($workgroupact) && $data['reference_activityid']==0){*/
    				...
    				$datarows.='<div class="col-md-12 activitiess activitycontent" id="enggactivities'.$data['id'].'"  data-id="'.$data['id'].'"><div style="padding:0px !important;margin-right:0px;" class="row " id="enggactivities'.$data['id'].'"  data-id="'.$data['id'].'">
                                    ...
                                    <div class="col-md-2 icon-groups" style="bottom: 3px;">
    									...
    									<a  class="btn btn-primary icon-pencil editscheduleactivitybutton" data-v="' . $data['id'] . '" value="' . $data['id'] . '" id="edit'.str_replace(' ','',$data['process']).'activitybut' . $data['id'] . '" data-type="'.$data['process'].'" title="Edit Activity" href="javascript:void(0);"></a>
    ...
    public function actionWbsestimatetasklists() 
    {
        ...
        if($projuser){
            ...
            $form = '<form method="POST" action="" id="scheduleform" class="col-md-12">
                       <table cellpadding="0" cellspacing="0 " id="activitytasktable" class="table"> 
                            ...
                            <tr>
                                ...
                                <td style="width: 35%;" class="taskleftline"><b>Task</b></td>
                                <td style="width: 35%;"><b>Duration(Hours)</b></td>
            ...
            if(count($tasks)>0){
                foreach ($tasks AS $key => $task):
                    ...
                    if($key==0){
                        ...
                        $buttonrow = '<div class="icon-groups" style="float:left;">
                    ...
                    $form .= '  <tr class="tastrow" id="activityrow' . $key . '">
                                ...
                                <td style="width: 35%;" class="taskleftline">
                                    ...
                                    <input type="text" class="form-control taskname_edit" name="taskname[]" value="' . $task['task'] . '">
                                </td>
                                <td style="width: 35%;">
                                    <input type="text" class="form-control taskduration_edit" name="taskduration[]" value="'.$sche_tasks['Budgeted_Duration'].'">
            ...
            $form .= '<tr class="activitylist">
                    <td style="width: 10%;" class="taskblankrow"></td>
                    <td style="width: 35%;" class="taskleftline"><b>Cycle time</b></td>

                    <td  style="width: 35%;" class="cycletimestyle"><span id="cyclebud">' . $btime . '</span> <input type="hidden" name="totalbudget" id="budcycle" value="' . $btime . '"></td>
                    <!--<td  style="width: 25%;"><span id="cycleact">' . $atime . '</span> <input type="hidden" name="aduration" id="actcycle" value="' . $atime . '"></td>-->
                    <td style="width: 10%;" class="taskrightline"></td>
                    <td style="width: 10%;" class="taskblankrow"></td>
                    </tr>';
        
            $sqlforactschedule = "SELECT Resourceunits,Workhours,Cycles FROM schedule_activity_new WHERE actvity_id='".$activityid."' AND activity_name='".$name."' ";
            ...
            $form .= '  <tr>
                                <td style="width: 10%;" class="taskblankrow"></td>
                                <td style="width: 35%;" class="taskleftline"><b>Number of Resource units</b></td>
                                <td style="width: 35%;"><b>Number of Cycles</b></td>
                                <td style="width: 10%;" class="taskrightline"></td>
                                <td style="width: 10%;" class="taskblankrow"></td>
                            </tr>';
            $form .= '  <tr>
                                <td style="width: 10%;" class="taskblankrow"></td>
                                <td style="width: 35%; border-bottom: 1px solid #ddd;" class="taskleftline">';
                                if($wbsactivity):
                                    ...
                                    }else
                                    {
                                        ...
                                        $form .= '<input style="" type="text" class="form-control" name="resourceunit" id="resourceunit" value="' . $sch_res . '"><span class="error"></span>';

                $form .= '      </td>
                                <td style="width: 35%; border-bottom: 1px solid #ddd;" >
                                        <input type="text" class="form-control" name="cycles" id="cycles" value="' . $sch_cyc . '"><span class="error"></span>
            ...
            $form .= '</tbody>
                    ...
                    <div class="text-center">
                        ...
                        <button type="button" class="btn btn-primary" id="saveschedule_new" name="saveschedule"><span class="icon-check"></span>Save</button>
    ...
    public function actionSchedulecreate()
    {
        ...
        $sql = 'INSERT INTO schedule_activity_new ( actvity_id,wbs_activity_Id,Project_Id, process_id, wbs_id,Resourceunits,Workhours,Cycles,progress,duration,start_date,end_date,activity_name )
                    VALUES
                    ( "'.$actid.'","0","'.$wrk['projectId'].'","0","0","'.$_POST['resourceunit'].'","'.$_POST['wrkhrs'].'","'.$_POST['cycles'].'","0","0","'.$startdate.'","'.$enddate.'","'.$activityname.'")';
        $command=$connection->createCommand($sql);
        $dataReader=$command->query();
        ...
        if($_POST['totalbudget']!=0 && $_POST['totalbudget']!=''){
            ...
            $wrk->actual_end_date = date('Y-m-d', strtotime($wrk->actual_start_date. ' + '.$dateadjust.' days')); 

            $wrk->save(false);
        }

        $this->GetRelationcorrect($wrk->projectId);
	...
	public function actionScheduleactivityupdate()
    {
      ...
      /*  if($_POST['lag']!=''){
        ...
        $this->GetRelationcorrect($activity1['projectId']);
	...
	public function GetRelationcorrect($projectid)
    {
        ...
        if(!empty($scheduleactivities))
        {
            foreach ($scheduleactivities as $scheduleactivity) 
            {
                ...
                $lastreported = "SELECT * from schedule_progress_report_log WHERE activity_id='".$activityid."' AND currentqty!=0 ORDER BY report_date DESC LIMIT 0,1";
                $command = $connection->createCommand($lastreported);
                $dataReader = $command->query();
                $lastreportedate = $dataReader->read();
                if(!empty($lastreportedate)){
                    $lastrprt = $lastreportedate['report_date'];
                }else{
                    $lastrprt = '';
                }
                //if($schedulereport->start_date > $scheduleactivity->actual_start_date && count($schedulereport)>0){
                ...
                //if($totalQty['totalqty']==$scheduleactivity->quantity){
                if($totalQty['totalqty']>0){
                    $datediff = strtotime($lastrprt) - strtotime($start_date1);
                }
                else{
                    $datediff = strtotime($today) - strtotime($start_date1);
                }
                $actual_duration = round($datediff / (60 * 60 * 24)) + 1;

                $duration_first = $scheduleactivity->old_duration;

                if($totalQty['totalqty']>0 && $today>=$start_date){
                    $actual_perdayqty = $totalQty['totalqty'] / $actual_duration;
                    $no_of_days = round($scheduleactivity->quantity / $actual_perdayqty);
                }
                else{
                    if($duration_first!=0){
                        $no_of_days = $duration_first;
                    }
                    else{
                        $no_of_days = $scheduleactivity->duration;
                    }
                }
        ...
        //$availableActivites = Scheduleactivities::model()->findAll(array('condition'=>'projectId ='.$projectid.' AND status =0','order'=>'scheduleitem_id ASC,sortorder ASC'));
   ...
   public function GetRelationcorrectfinal($projectid)
    ...
    public function actionActivityrelation()
    {
        ...
        if($projuser)
        {
            ...
            $datarows.='<div class="row relationlist">
                        ...
                        <div class="col-md-2">			
                            <div class="card relations-card">
                                ...
                                <div class="card-body">
                                    <div class="radio">
                                        <label><input type="radio" class="relation_type" name="relation_type" value=1>SS</label>
                                    </div>
                                    <div class="radio">
                                        <label><input type="radio" class="relation_type" name="relation_type" value=2>FS</label>
                                    </div>
                                    <div class="radio">
                                        <label><input type="radio" class="relation_type" name="relation_type" value=3>FF</label>
                                    </div>
            ...
            if($_POST['Relactionename'] != ''){
                ...
                $relations = ActivityRelations::find()->where(['projectId' => $prjct_id])->andWhere(['status' => 0])->orderBy(['sortorder' => SORT_ASC])->all();
            }
             $html = '';
            if(!empty($relations))
            {
                    ...
                    if($prec_item->status==0 && $dep_item->status==0 && $prec_actvty->status==0 && $dep_actvty->status==0){
                        ...
                        <div class="row relationlist" id="relationrow'.$relation['id'].'" style="cursor: pointer;min-height: 100px;" data-id="'.$relation['id'].'">
                                    ...
                                    <div class="col-md-1 icon-groups" style="bottom:14px;">				
                                        ...
                                        <a class="btn btn-danger  icon-trash1 deleterelation" data-v="'.$relation['id'].'" value="'.$relation['id'].'"  id="deleterelationbutton'.$relation['id'].'" title="Delete Relation" '.$relationstyle.' href="javascript:void(0);"></a>
                                    </div>
                                </div>
            ...
            $arr = array('result' => $datarows,'relationList' => $html,'error'=>'No','selecteditemone' => $selecteditemone,'selecteditemtwo' => $selecteditemtwo);
            return json_encode($arr);
    ...
    public function actionSaverelation()
    {
        ...
        if($projdur >= $duration_first){
            ...
            if($precedentAct->start_date=='' && $precedentAct->end_date==''){
             $precedentAct->start_date = date('Y-m-d');
             $precedentAct->end_date = date('Y-m-d');
             $precedentAct->actual_start_date = date('Y-m-d');
             $precedentAct->actual_end_date = date('Y-m-d');
            ...
            if($precedentAct->start_date!='' && $precedentAct->end_date!='')
            {
                ...
                if($model->save(false))
                {
                    ...
                    $this->GetRelationcorrect($_POST['projectId']);
    ...
    public function actionUpdaterelation()
    {
        ...
        if($model->save(false)){
            ...
            $this->GetRelationcorrect($model->projectId);
                        ...
                        foreach($wbsactivities as $key=> $wbsactivity):
                            ...
                            $datarows.='<tr  id="Estimateiowactivitiesrow' . $wbsactivity['id'] . '" data-id="' . $wbsactivity['id'] . '">
                                        ...
                                        <span class="error"></span></td>
                                            ...
                                            <td><input type="text" data-type="'.$processname.'" data-iow="'.$wbsactivity['wbs_id'].'" data-id="' . $wbsactivity['id'] . '" class="form-control quantity" name="quantity[]" id="'.str_replace(' ','',$processname).'quantity' . $wbsactivity['id'] .'" value="'. $activityqty.'" >
                                        <span class="error"></span></td>
        ...
        if (isset($_POST['itemid'])) {
            ...
            if($estimatecount > 0):
                for ($i = 0; $i < count($_POST['itemid']); $i++) {
                    ...
                    $estimateactcount=PricingEstimateNew::find()->where(['project_Id'=>$_POST['Project_Id']])->andWhere(['activity_Id'=>$itemid])->count();
                    if($estimateactcount > 0):
   ...
   public function actionRemoveitemold()
   {
    ...
    public function actionNewganttchart($id)
    {
    ...
    public function actionGetcriticalpath()
    {
        ...
        //echo $dependentAct->id; exit;
        //$dependentAct = Scheduleactivities::model()->findByPk($_POST['activityid']);
        //echo $_POST['activityid']; exit;
        //$availableActivites = Scheduleactivities::model()->findAll(array('condition'=>'scheduleitem_id ='.$dependentAct->scheduleitem_id.' AND status =0','order'=>'sortorder DESC'));

        $availableActivites = Scheduleactivities::find()->where(['projectId' => $dependentAct->projectId])->andWhere(['status' => 0])->orderBy(['end_date' => SORT_DESC])->all();

        //print_r($availableActivites); exit;

        //$this->Getbackwardpass($dependentAct->projectId);
        ...
        if(!empty($availableActivites))
        {
            ...
            function GetId($z,$newarray){
                ...
                $allRelations1 = ActivityRelations::find()->Where(['dependent_activity' => $z])->andWhere(['status'=>0])->all();
                    ...
                    //$allRelations2 = ActivityRelations::model()->findAll(array('condition'=>'precedent_activity ='.$x.' AND status =0'));
                    ...
                    if(!empty($f)){
                        //$newarray[]= $y;
                        return GetId($f,$newarray);
                    }
            
                  return($newarray);         //echo $newids;
            //die();
            ...
            $x = $dependentAct->id;
            //$newarray[] = $x;
            $allRelations1 = ActivityRelations::find()->Where(['dependent_activity' => $x])->andWhere(['status'=>0])->all();
            ...
            foreach($allRelations1 as $relation):
                ...
                if($precAct1->critical_start == $precAct1->start_date){
                    $dependentid = $relation->precedent_activity;
                    //$x = $relation->precedent_activity;
                    $y = $relation->precedent_activity;
                    //array_push($Ids, $y);
                    $idarray = GetId($y,$newarray);
                     //print_r($idarray);
                } 
            endforeach;

            //print_r($idarray);
            //} while (count($allRelations1)==0);
        ...
        //echo count($Ids); exit;
       // if(count($Ids)>1){
        $arr=array('error'=>'No','criticalIDs' => $criticalids);
        return json_encode($arr);
       // }
    }
       ...
       // $query = "SELECT * FROM wbsscheduleitems WHERE projectId='" . $_POST['projectid']. "' AND status=0 ORDER BY sortorder ASC";
        ...
        foreach($items as $item){
            ...
            // var_dump($array);
             ...
             $command = $connection->createCommand($sql);
             $dataReader = $command->query();
             $edndate = $dataReader->read();
             
                 $strtdatee=$strtdate['start_date'];
                 $edndatee=$edndate['end_date'];

                 $datediff = strtotime($edndatee) - strtotime($strtdatee);
                 $totduration = round($datediff / (60 * 60 * 24)) + 1;
                 //print_r($duration_first);exit;
    ...
    public function actionGetitemactivites()
    {
        ...
        $sql="  SELECT s.* FROM workgroup_activities_new as w 
 ...
 // echo "*ys cart-els-sts-0-cart-*";                                           
                                    ...
                                    }elseif($estimateresource->resourcetype_Id == 26)
                                    {
                                        ...
                                        if(empty($Jobcardd)){
                                            ...
                                            $Jobcarddfuel=Jobcard::find()->where(['vendor_id'=>$resource->Vendor_Id])->andWhere(['in','resource',[183,184]])->andWhere(['app_activity'=>$estimateresource->activity_id])->andWhere(['delete_status'=>0])->andWhere(['pricing_resourceid'=>$estimateresource->pricing_resourceid])->one();
                                            ...
                                            if(empty($Jobcarddfuel)){
                                                ...
                                                if(!$jobfindfuel)
                                                {
                                                    ...
                                                    if($Jobcard->save(false)):
                                                        ...
                                                        $model->Job_Id= $Jobcard->job_id;
                                                        if($estimateresource->resourcetype_Id == 26 && $estimateresource->lease_period == null){                      
                                                            $model->Vendor_Id= '9';                                               
                                                        }else{
                                                            $model->Vendor_Id= $Jobcard->vendor_id;
                                                        }
                                                ...
                                                foreach($fuelestimateress as $estimateres){
                                                   ...
                                                   // if($estimateres->resourcetype_Id==26 && $estimateres->fuel_type != ''){
                                                        ...
                                                        $count=PricingEstimateResourcesNew::find()->where(['resource_Id'=>$estimateresource->resource_Id])->andWhere(['project_id'=>$estimateresource->project_id])->andWhere(['pricing_status'=>0])->andWhere(['resourcetype_Id'=>24])->andWhere(['diesel_orderd_status'=>0])->count();                                                 
