...
class Activity1Controller extends Controller
{
    ...
    public function actionListactivities()
    {
        ...
        if(count($wbsactivities)>0):
            $datarows='';
            $datarows.='<div class="row wbsheadsss">
                            <div class="col-md-4">
                                <div class="row">
                                    <div class="col-md-2">
                                        <label>#</label>
            ...
            foreach($wbsactivities AS $key=>$wbsactivity):
				...
				if($wbsactivity['boq_slno']==0){
                    $boqs = Boq::find()->where(['projectid' => $projid])->andWhere(['wbs' => $wbsactivity['wbs_id']])->orderBy(['slno' => SORT_ASC])->all();
					$boqslno = '<select id="boqslno" name="boqslno[]" class="form-control"><option value="0">Select BOQ No</option>';
					foreach($boqs as $boq):
                     $wbs=WorkgroupsNew::find()->where(['Workgroup_Id' => $boq->wbs])->andWhere(['Status' => 0])->andWhere(['pricing_status' => 0])->one();
					 if($boq->slno!=0 && !empty($wbs)){
						 $boqslno.="<option value=".$boq->boq_id.">".$boq->slno."</option>";
					 }
					endforeach;
                    ...
                    $boqs = Boq::find()->where(['projectid' => $projid])->andWhere(['wbs' => $wbsactivity['wbs_id']])->orderBy(['slno' => SORT_ASC])->all();
                    //$boqs = Boq::find()->where(['projectid' => $projid])->orderBy(['slno' => SORT_ASC])->all();
					$sl_no = Boq::findOne($wbsactivity['boq_slno']);
                    if($sl_no){
                        $boqslno = '<span id="boq_slno'.$wbsactivity['id'].'">'.$sl_no->slno.'</span><select id="boqslno'.$wbsactivity['id'].'" name="boqslno[]" class="form-control" style="display:none;"><option value="0">Select BOQ No</option>';
                    }
                    else{
                        $boqslno = '<span id="boq_slno'.$wbsactivity['id'].'"></span><select id="boqslno'.$wbsactivity['id'].'" name="boqslno[]" class="form-control" style="display:none;"><option value="0">Select BOQ No</option>';
                    }
					
					foreach($boqs as $boq):
                     $wbs=WorkgroupsNew::find()->where(['Workgroup_Id' => $boq->wbs])->andWhere(['Status' => 0])->andWhere(['pricing_status' => 0])->one();
                     //$wbs=WorkgroupsNew::find()->where(['Workgroup_Id' => $wbsactivity['wbs_id']])->andWhere(['Status' => 0])->andWhere(['pricing_status' => 0])->one();
					 if($boq->slno!=0 && !empty($wbs)){
						 if($boq->slno==$sl_no->slno){
						 	$selected = 'selected';
						 }
						 else{
						 	$selected = '';
						 }
						 $boqslno.="<option value=".$boq->boq_id." $selected>".$boq->slno."</option>";
					 }
					endforeach;
                ...
                if($wbsactivity['boq_slno']!='' && $wbsactivity['boq_slno']!=0){
                    $boq_slno1 = $wbsactivity['boq_slno'];
                    $boq_slno2 = Boq::findOne($boq_slno1);
                    if($boq_slno2){
                        $boq_slno = $boq_slno2->slno;
                    }
                    else{
                        $boq_slno = '';
                    }
                ...
                $datarows.='<div style="padding: 0px;" class="row prjactivityss" id="iowactivitiesrow'.$wbsactivity['id'].'"  data-id="'.$wbsactivity['id'].'" data-type="'.$wbsactivity['process_Id'].'">
                                <div class="col-md-4">
                                    <div class="row" style="padding: 0px;">
                                        <div class="col-md-2 frstnmm" style="padding-top: 11px;">
                                            ...
                                            <input type="hidden"  id="estimate" name="wbsactid[]" value = "'. $wbsactivity['id'] . '">
                                            <input type="hidden"  id="sortorder" name="sortorder[]" value = "'. $wbsactivity['sortorder'] .'">
                                        </div>
                                        <!--<div class="col-md-2 type boqnam">
                                            
                                            <span>'.$boqslno.'</span>
                                        </div>-->
                                        <div class="col-md-5 type bqactnme" style="padding-top: 11px;">
                                ...
                                <div class="col-md-8" style="padding: 0px;">
                                    <div class="row" style="padding:0px;">
                                        <div class="col-md-2 type boqnmm" style="padding-top: 11px;padding-left: 23px;">
                                            
                                            <span id="Boqslno'.$wbsactivity['id'].'">'.$boq_slno.'</span>
                                        </div>
                                        <div class="col-md-3 type boqunut" style="padding-top: 11px;">
                                        ...
                                        <div class="col-md-2 type boqamnttt" style="text-align: right;padding-top: 11px;">
                                            
                                            <span>'.number_format($specrate,2).'</span>
                                        </div>
                                        <div class="col-md-5 icon-groups" style="bottom: 4px;">
                                            
                                            <a class="btn btn-primary text-button mapactivity" data-v="'. $wbsactivity['id'] .'" value="'. $wbsactivity['id'] .'" data-id="'.$wbsactivity['activity_Id'].'" id="mapactivity' . $wbsactivity['id'] . '" title="Map BOQ"  href="javascript:void(0);"><span class="icon-dns"></span>Map BOQ</a>
    ...
    public function actionUpdateiowactivity()
    {
        ...
        $model2=Scheduleactivities::find()->where(['activity_id'=>$iowactid])->andWhere(['status'=>0])->one();
        $model->activity_Name=$_POST['name'];
        $model->activity_Unit=$_POST['unit'];
        $model->estimate=$_POST['estimate'];
        if(!empty($_POST['slno'])){
            $model->boq_slno=$_POST['slno'];
        }
        $model->save(false);
    ...
    public function actionAddiowactivity()
    {
        ...
        //if($activityname=="Fabrication of Liners"){ 

        if($activity && $activity->activity_Name=="Fabrication of Liners"){
           $activitytype_id=2; 
           // echo "yes"; exit;
        }
        //$actsql="SELECT * FROM workgroup_activities_new WHERE activity_status=0";
        //print_r($activityname); exit;
        ...
        $sql = 'INSERT INTO workgroup_activities_new (project_Id,wbs_id,worktype_Id,activitytype_id,activity_Id,activity_Unit,estimate,amount,sortorder,type,activity_Name,process_Id,schedule,duration,cycle_Unit,cycle_Quantity,pr_status,pricing_status,operations_status,wbs_estimate_id)
                    VALUES
                    ( "'.$projectid.'","'.$wrkgrpid.'","'.$worktype_Id.'","'.$activitytype_id.'","'.$activid.'","'.$activity_Unit.'","1","'.$amount.'","'.$sortorder.'","'.$typeval.'","'.$_POST['activityname'].'",0,0,0,0,0,0,0,0,0)';
        //echo $sql;exit;
        $command=$connection->createCommand($sql);
        $dataReader=$command->query();
