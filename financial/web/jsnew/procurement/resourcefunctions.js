...
/* 
        ...
        if(error==0){
            $.ajax({
                ...
                dataType:'json',
                data: {resourcename:$('#resourcename').val(),schedules:schedules,restype:restype,accountsubgrp:$('#accountsubgrp').val(),accounthead:$('#accounthead').val(),addresourcetypeaccount:$('#addresourcetypeaccount').val(),vendor:$('#vendor').val(),unit:$('#resourceunit').val(),rate:machinerate,vendorlocation:$('#vendorlocation').val(),addProcess:$('#addProcess').val(),resgroup:$('#resgroup').val(),resourcequantity:$('#resourcequantity').val(),vendorphone:$('#vendorphone').val(),vendoremail:$('#vendoremail').val(),brand:$('#brand').val(),equconsumption:$('#equconsumption').val(),equrate:$('#equrate').val(),equmaintenancecost:$('#equmaintenancecost').val(),machinerate:$('#machinerate').val(),equuunit:$('#equuunit').val()},
                success:function(data){
                    if(data.error=='No')
                    {
