...
$(document).on( "click", ".estimate-addresource", function(){  
    ...
    if(resourcetypeid==26){ 
        ...
        var type = document.querySelector('input[name="pequ-select"]:checked').value;
    ...
    //}
        ...
        var type = document.querySelector('input[name="equ-select"]:checked').value;
        ...
        if (type == 'no_of_hrs') {
            ...
            $.ajax({
                ...
                dataType: "json",
                data: {type:type,repair:repair,batchingunit:batchingunit,fueltype:fueltype,fuelrate:fuelrate,fuelqty:fuelqty,hoursno:hoursno,resourcetypeid:resourcetypeid,Product_Name:Product_Name,Product_Unit:Product_Unit,resourceid:resourceid,Process:Process,activityid:activityid,project_estimate_Id:project_estimate_Id,Project_Id:Project_Id},
                success: function(data){
                    if(data.error=='No')
                    {
    ...
    { 
        ...
        if(act_qty == 0 || act_qty == null)
        {
            ...
            if(error==0)
            {
                $.ajax({
                    ...
                    dataType: "json",
                    data: {nodays:nodays,mannumber:mannumber,mandays:mandays,resourcetypeid:resourcetypeid,Product_Name:Product_Name,Product_Unit:Product_Unit,resourceid:resourceid,resourcerate:resourcerate,resourceqty:resourceqty, Process:Process,activityid:activityid,project_estimate_Id:project_estimate_Id,Project_Id:Project_Id},
                    success: function(data){
                        if(data.error=='No')
                        {
        ...
        if(error==0)
        {
            $.ajax({
                ...
                dataType: "json",
                data: {resourcetypeid:resourcetypeid,Product_Name:Product_Name,Product_Unit:Product_Unit,resourceid:resourceid,resourcerate:resourcerate,resourceqty:resourceqty, Process:Process,activityid:activityid,project_estimate_Id:project_estimate_Id,Project_Id:Project_Id},
                success: function(data){
                    if(data.error=='No')
                    {
...
$(document).on("change", "select[type=text][name=fuelstyp]", function () {
...
$(document).on('change','.mannumbr',function(){
    var id = $(this).data('id');
...
$(document).on('change','.ofdays',function(){
    var id = $(this).data('id');
    $('.btnclass'+id).prop('disabled',false);
    var manno = $('#estimate-mannumbr'+id).val();
    var nodays = $('#estimate-no-ofdays'+id).val();
