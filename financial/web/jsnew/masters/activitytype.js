...
$(document).on('click','.childestactivities',function(){
    var activitytypeid=$(this).data("id");
    $('#searchestactivitytype').val(activitytypeid);
    $('.prjct-estactvts').trigger('click');
});
...
$( "#activitytypeitems" ).sortable({
        ...
        update:function( event, ui ) {
            ...
            $.ajax({
                type: 'POST',
                url: '../projects/updateestactivitytypesort',
                data: {datavalue:updatedrows},
                dataType: "json",
                success: function(data){
                    $('#activitytypesearch').trigger('click');
                }
            });
        }
