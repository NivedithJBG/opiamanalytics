...
class Sensational_Blog_Dropdown_Chooser extends WP_Customize_Control{
	...
	public function render_content(){
		...
		?>
            <label>
                ...
                <select class="sensational-blog-chosen-select" <?php $this->link(); ?>>
                    <?php
...
class Sensational_Blog_Dropdown_Multiple_Chooser extends WP_Customize_Control{
    ...
    public function render_content(){
        ...
        ?>
            <label>
                ...
                <select class="sensational-blog-chosen-select" multiple <?php $this->link(); ?>>
                    <?php
                    foreach ( $this->choices as $value => $label )
                        echo '<option value="' . esc_attr( $value ) . '"' . selected( $this->value(), $value, false ) . '>' . esc_html( $label ) . '</option>';
                    ?>
...
Class Sensational_Blog_Customize_Horizontal_Line extends WP_Customize_Control {
    public $type = 'hr';

    public function render_content() {
        ?>
        <div>
            <hr style="border: 2px dashed #6caf17;" />
        </div>
        <?php
    }
}

class Sensational_Blog_Multi_Input_Custom_Control extends WP_Customize_Control {
    /**
     * Control type
     *
     * @var string
...
class Sensational_Blog_Note_Control extends WP_Customize_Control {
