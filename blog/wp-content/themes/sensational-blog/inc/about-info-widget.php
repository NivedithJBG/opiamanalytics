...
if ( ! class_exists( 'Sensational_Blog_About_Us_Image_Widget' ) ) :

	/**
	 * Adds Tp image widget.
	 */
	class Sensational_Blog_About_Us_Image_Widget extends WP_Widget {
		...
		public function __construct() {
			$widget_ops = array(
				...
				'description' => esc_html__( 'An widget to upload About Content and images in sidebar.', 'sensational-blog' ),
			);
			parent::__construct( 'widget_about', esc_html__('About Us Image','sensational-blog'), $widget_ops );
		}
				    ...
				    ?>
		                ...
		                <div class="social-icons">
		                    <ul>
								...
								?>
							        <li>
							        	<a href="<?php echo esc_url( $link ) . '" ' . esc_attr( $target ); ?>"><?php echo sensational_blog_return_social_icon( $link ); ?>
							        	</a>
							        </li>
								<?php endif;
								} ?>
		...
		public function form( $instance ) {
			...
			?>
			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'title' ) ); ?>"><?php esc_html_e( 'Title :', 'sensational-blog' ); ?></label>
			...
			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'name' ) ); ?>"><?php esc_html_e( 'Name :', 'sensational-blog' ); ?></label>
			...
			<div>
				<label for="<?php echo esc_attr( $this->get_field_id( 'tpiw_image_url' ) ); ?>"><?php esc_html_e( 'Image URL', 'sensational-blog' ); ?></label>:<br />
				...
				<input type="button" class="select-img button button-primary" value="<?php esc_attr_e( 'Upload', 'sensational-blog' ); ?>" data-uploader_title="<?php esc_attr_e( 'Select Image', 'sensational-blog' ); ?>" data-uploader_button_text="<?php esc_attr_e( 'Choose Image', 'sensational-blog' ); ?>" style="margin-top:5px;" />
		      	...
		      	<div class="tpiw-preview-wrap" <?php echo esc_attr( $wrap_style ); ?>>
		        	<img src="<?php echo esc_url( $full_image_url ); ?>" alt="<?php esc_attr_e('Preview', 'sensational-blog'); ?>" style="max-width: 100%;"/>
		      	</div><!-- .tpiw-preview-wrap -->

	    	</div>

		   <p>
			   	<label for="<?php echo esc_attr( $this->get_field_id( 'number' ) ); ?>"><?php esc_html_e( 'Number of links to show:', 'sensational-blog' ); ?></label>
		   ...
		   <p>
		    	<label for="<?php echo esc_attr( $this->get_field_id( 'open_link' ) ); ?>"><?php esc_html_e( 'Open Social link in New Tab', 'sensational-blog' ); ?>:</label>
		      	<input type="checkbox" id="<?php echo esc_attr( $this->get_field_id( 'open_link' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'open_link' ), 'sensational-blog' ); ?>"  <?php checked( $open_link, true ); ?> />
		   </p>

		   <?php for ( $i=1; $i <= $number; $i++ ) {
		   	$link = isset( $instance['link'. '-' . $i ] ) ? $instance['link' . '-' . $i ] : '';?>
			   <p>
				   	<label for="<?php echo esc_attr( $this->get_field_id( 'link' . '-' . $i ) ); ?>"><?php printf( esc_html__( 'Link %s :', 'sensational-blog' ), $i ); ?></label>
		...
		public function update( $new_instance, $old_instance ) {
			...
			$instance['open_link']       	= sensational_blog_sanitize_checkbox( $new_instance['open_link'] );
 ...
 * @since Blog Diary Pro 1.0
 */
function sensational_blog_about_us_image_widget_upload_enqueue( $hook ) {

  if( 'widgets.php' !== $hook )
      return;

  wp_enqueue_media();
  wp_enqueue_script( 'sensational-blog-image-widget-upload-script', get_template_directory_uri() . '/assets/js/upload.min.js', array( 'jquery' ), '1.1', true );

}
add_action( 'admin_enqueue_scripts', 'sensational_blog_about_us_image_widget_upload_enqueue' );