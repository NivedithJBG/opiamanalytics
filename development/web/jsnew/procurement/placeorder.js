...
/*$(document).on( "click", ".addvendortocart", function(){
    ...
    $('#collapseplaceord').addClass('in');

    $("#collapsevendors").attr("aria-expanded","false");

    $("#collapseplaceord").attr("aria-expanded","true");

    $('#collapseplaceord').css('height','');
    ...
    $('#choooseplaceorder').trigger('click');
...
$(document).on( "click", "#choooseplaceorder", function(){
...
$(document).on('click','#pordritems input',function(){
    var values = [];
    var ordervalues = [];
    var restype = [];
    var project = [];
    $('#pordritems input:checked').each(function() {
        values.push($(this).val());
        ordervalues.push($(this).attr('data-id'));
        restype.push($(this).attr('data-restype'));
        project.push($(this).attr('data-project'));
    });
    $('[name="vendorrs"]').attr({value: values.join(', ')});
    $('[name="ordeerrs"]').attr({value: ordervalues.join(', ')});
    $('[name="restyype"]').attr({value: restype.join(', ')});
...
$(document).on('click','#placeorderss',function(){

    if($('.vendors').is(":checked")) {
       var selvendors=$('#vendorrs').val();
       var restype=$('#restyype').val();
       var orders=$('#ordeerrs').val();
         ...
         $.ajax({
            ...
            if( vendorname=="Company Owned" && restype!=33){
                ordertype="Despatch Order";
                 ...
                 $('#despatchorderdatas').html(data.despatchorder);  
                 $('#despatchorderdatas').show();
     ...
     $.ajax({
            ...
            success: function (data) {
                if (data.error == 'No') {
                    ...
                    if(restype==19){   
                  ordertype="Work Order";  
                $('#workorderdatas').html(data.workorder);  
                 $('#workorderdatas').show();
            }
            else if(restype==24){ 
                ordertype="Lease Order";  
                 $('#leaseorderdatas').html(data.leaseorderr);  
                 $('#leaseorderdatas').show();

            }
            else if(restype==33){
                ordertype="Direct Work Order";
                $('#musterrolldatas').html(data.musterroll);  
                 $('#musterrolldatas').show();
            ...
            // }
                ...
                ordertype="Purchase Order";
                 $('#purchaseorderss').html(data.purchaseorder);  
                 $('#purchaseorderss').show();
            }
                    //$('#resourcessearch').trigger('click');  
...
$(document).on('click','#cancelordeer',function(){   


      
         $('#purchaseorderss').hide();
            
          $('#placeorderdatas').show();
 ...
 $(document).on('click','#cancelorderworkorderr',function(){   
 ...
 $(document).on('click','#cancelorderleaseordeer',function(){   


      
         $('#leaseorderdatas').hide();
  ...
  $(document).on('click','#cancelordermusterrolll',function(){   


      
         $('#musterrolldatas').hide();
  ...
  $(document).on('click','#cancelorderdespatchs',function(){   


      
         $('#despatchorderdatas').hide();
